...
int main() {
    int n = io.nextInt();
    auto cost = io.nextLongs(n);

    Memo getMinCost(-1LL, n+2, 2);
    getMinCost = [&](int start, int lastOn) -> Long {
        if (start >= n || (lastOn && start == n-1)) {
            return 0;
        }
        return std::min(cost[start] + getMinCost(start+2, lastOn),
                        cost[(start-1+n)%n] + getMinCost(start+1, (start == 0 ? true : lastOn)));
    };

    io.println(getMinCost(0, false));
