...
int main() {
    io.codeforces([&]() {
        ...
        int arrMax = *std::max_element(arr.begin(), arr.end());

        int answer = INT32_MAX;
        for (int v = 0; v <= arrMax; ++v) {
            int globalMax = 0, globalMin = INT32_MAX;
            bool valid = true;
            for (int i = 0; i < n; ++i) {
                int p = arr[i] / (v+1) + 1;
                if (p > k) {
                ...
                globalMax = std::max(globalMax, arr[i]/p);
                globalMin = std::min(globalMin, arr[i]/p);
            }
            if (valid) {
                answer = std::min(globalMax-globalMin, answer);
            }
