...
int main() {
    static const std::set<char> VOWEL = { 'a', 'e', 'i', 'o', 'u' };
    ...
    io.google([&]() {
        ...
        auto isSpell = [&]() -> bool {
            for (int s = 0; s+4 < vowels.size(); ++s) {
                for (int e = s+3; e+1 < vowels.size(); ++e) {
                    std::pair<int, int> start = { vowels[s], vowels[s+1] };
                    std::pair<int, int> end = { vowels[e], vowels[e+1] };

                    if (str[start.first] == str[end.first]
                    && str[start.second] == str[end.second]
                    && start.second-start.first == end.second-end.first
                    && str.substr(start.first, start.second-start.first+1) == str.substr(end.first, end.second-end.first+1)) {
                        return true;
                    }
                }
            }
        ...
        io.println(isSpell() ? "Spell!" : "Nothing.");
    });
