...
int main() {
    ...
    io.codeforces([&]() {
        ...
        Tensor<2, int> lShapeId = createTensor(0, n, m);
        int lId = 1;

        auto match = [&](int mr, int mc, const Shape& sh) {
            for (int i = 0; i < sh.size(); ++i) {
                for (int j = 0; j < sh[i].size(); ++j) {
                    if (lShapeId[mr+i][mc+j] || sh[i][j] != map[mr+i][mc+j]) {
                        return false;
                    }
                }
            }
        ...
        for (int i = 0; i+1 < n; ++i) {
            for (int j = 0; j+1 < m; ++j) {
                for (int s = 0; s < L_SHAPES.size(); ++s) {
                    match(i, j, L_SHAPES[s]);
                }
            }
        }
            ...
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < m; ++j) {
                    ...
                    for (int k = 0; k < 8; ++k) {
                        int r = i + dr[k];
                        int c = j + dc[k];
                        if (r < 0 || c < 0 || r >= n || c >= m || map[r][c] == '.' || lShapeId[r][c] == lShapeId[i][j]) {
                            continue;
                        } else {
                            return false;
                        }
