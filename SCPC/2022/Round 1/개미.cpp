...
int main() {
    io.samsung([&]() {
        ...
        auto v = io.nextInts(n);

        std::map<int, std::set<int>*> pointsByValue;
        for (int i = 0; i < n; ++i) {
            if (pointsByValue.count(v[i]) == 0) {
                pointsByValue[v[i]] = new std::set<int>();
            }
            pointsByValue[v[i]]->insert(p[i]);
        }

        Long answer = 0;
        for (int i = 0; i < n; ++i) {
            auto currBucketIt = pointsByValue.begin();
            auto currBucket = currBucketIt->second;
            auto right = currBucket->lower_bound(p[i]);
            auto left = (right == currBucket->begin() ? currBucket->end() : std::prev(right));

            if (left != currBucket->end()) {
                answer += p[i] - *left;
                currBucket->erase(left);
            } else {
                answer += *right - p[i];
                currBucket->erase(right);
            }

            if (currBucket->empty()) {
                pointsByValue.erase(currBucketIt);
            }
        }
        io.println(answer);
    });
