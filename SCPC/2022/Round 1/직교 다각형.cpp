...
int main() {
    io.samsung([&]() {
        ...
        std::map<int, std::vector<Point*>> pointByX, pointByY;
        for (int i = 0; i < n; ++i) {
            auto [x, y] = io.nexts<int, 2>();
            points[i] = { x, y };
            pointByX[x].push_back(&points[i]);
            pointByY[y].push_back(&points[i]);
        }

        for (auto& [x, ps] : pointByX) {
            std::sort(ps.begin(), ps.end(), [](Point* p1, Point* p2) { return p1->second < p2->second; });
        }
        for (auto& [y, ps] : pointByY) {
            std::sort(ps.begin(), ps.end(), [](Point* p1, Point* p2) { return p1->first < p2->first; });
        }

        auto getPerimeter = [&](Point* exclude) -> Long {
            Long result = 0;
            for (const auto& [x, ps] : pointByX) {
                std::vector<const Point*> cands;
                ...
                for (int i = 0; i+1 < cands.size(); i += 2) {
                    result += cands[i+1]->second - cands[i]->second;
                }
            }
            for (const auto& [y, ps] : pointByY) {
                std::vector<const Point*> cands;
                cands.reserve(ps.size());
                for (const Point* p : ps) {
                    if (p != exclude) {
                ...
                for (int i = 0; i+1 < cands.size(); i += 2) {
                    result += cands[i+1]->first - cands[i]->first;
                }
            }
            return result;
        };
        ...
        if (oddX != -1 && oddY != -1) {
            for (int i = 0; i < n; ++i) {
                if (points[i].first == oddX && points[i].second == oddY) {
                    io.println(getPerimeter(&points[i]));
                    break;
                }
            }
        } else {
            io.println(getPerimeter(nullptr));
        }
    });
