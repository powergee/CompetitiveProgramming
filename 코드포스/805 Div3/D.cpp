...
int main() {
    io.codeforces([&]() {
        auto origin = io.nextToken();
        int p = io.nextInt();
        std::vector<int> pos[26];
        int currScore = 0;
        for (int i = 0; i < origin.size(); ++i) {
            pos[origin[i]-'a'].push_back(i);
            currScore += origin[i]-'a'+1;
        }
        std::vector<bool> included(origin.size(), true);
        int posIdx = 25;
        while (p < currScore) {
            while (posIdx >= 0) {
                if (pos[posIdx].size()) {
                    currScore -= posIdx+1;
                    included[pos[posIdx].back()] = false;
                    pos[posIdx].pop_back();
                    break;
                } else {
                    posIdx--;
