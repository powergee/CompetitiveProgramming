...
int main() {
    io.google([&]() {
        ...
        std::vector<int> radius(padded.size(), 0);
        int maxRadius = 0;
        int maxCenter = 0;
        for (int i = 0; i < radius.size(); ++i) {
            if (i <= maxRadius) {
                radius[i] = std::min(radius[2*maxCenter-i], maxRadius-i);
            } else {
                radius[i] = 0;
            }

            while (i - radius[i] - 1 >= 0 &&
                   i + radius[i] + 1 < radius.size() &&
                   padded[i - radius[i] - 1] == padded[i + radius[i] + 1]) {
                radius[i]++;
            }

            if (maxRadius < i + radius[i]) {
        ...
        for (int i = 1; i < padded.size(); ++i) {
            if (radius[i] == i) {
                int nextStart = i + radius[i];
                int nextSize = padded.size() - nextStart;
                int nextCenter = nextStart + nextSize / 2;
                if (radius[nextCenter] == nextSize / 2) {
                    auto sub = padded.substr(0, radius[i]*2);
                    std::string answer = "";
                    for (char ch : sub) {
                        if (ch != '#') {
                            answer += ch;
