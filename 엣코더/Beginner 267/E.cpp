...
int main() {
    ...
    auto values = io.nextLongs(n, 1);
    auto graph = io.nextGraph(n, m);

    std::vector<Long> costs(n+1, 0);
    for (int u = 1; u <= n; ++u) {
        for (int v : graph[u]) {
            costs[v] += values[u];
        }
    ...
    std::function<bool(Long)> isPossible = [&](Long costBound) -> bool {
        std::vector<Long> currCost = costs;
        std::vector<bool> visited(n+1, false);
        std::queue<int> q;
        for (int v = 1; v <= n; ++v) {
            if (currCost[v] <= costBound) {
                q.push(v);
                visited[v] = true;
            }
        }
        ...
        while (!q.empty()) {
            ...
            for (int v : graph[u]) {
                currCost[v] -= values[u];
                if (!visited[v] && currCost[v] <= costBound) {
                    q.push(v);
                    visited[v] = true;
                }
            }
        ...
        return std::count(visited.begin()+1, visited.end(), false) == 0;
    };

    Long l = 0, r = std::accumulate(values.begin(), values.end(), 0LL);
    while (l < r) {
        Long mid = (l+r) / 2;
        if (isPossible(mid)) {
            r = mid;
