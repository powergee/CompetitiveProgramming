...
int main() {
    io.codeforces([&]() {
        int n = io.nextInt();
        auto tea = io.nextLongs(n);
        auto taster = io.nextLongs(n);

        Long sub = 0;
        std::vector<Long> answer(n, 0);
        std::multiset<Long> remaining = { tea[0] };
        for (int i = 0; i < n; ++i) {
            while (remaining.size() > 0 && *remaining.begin()-sub <= taster[i]) {
                answer[i] += *remaining.begin()-sub;
                remaining.erase(remaining.begin());
            }
            if (remaining.size()) {
                answer[i] += remaining.size() * taster[i];
                sub += taster[i];
            }
            if (i < n-1) {
                remaining.insert(tea[i+1]+sub);
            }
        }
