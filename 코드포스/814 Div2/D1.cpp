...
int main() {
    io.codeforces([&]() {
        ...
        Memo getMinTime(-1, n, (1<<13));
        getMinTime = [&](int start, int currXor) -> int {
            int acc = arr[start] ^ currXor;
            if (start == n-1) {
                return (acc == 0) ? 0 : 1;
            }
            if (acc == 0) {
                return getMinTime(start+1, 0);
            }
            for (int i = start+1; i < n; ++i) {
                if (acc != arr[i]) {
                    return std::min(div2Ceil(i-start)+getMinTime(i, 0), div2Ceil(i-start+1)+getMinTime(i, acc));
                }
            }
            return div2Ceil(n-start);
        };

        io.println(getMinTime(0, 0));
    });
