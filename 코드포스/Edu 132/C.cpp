...
int main() {
    io.codeforces([&]() {
        ...
        io.printYes([&]() -> bool {
            ...
            int opened = (int(quesIdx.size()) - fixed) / 2;
            int closed = quesIdx.size() - opened;
            ...
            for (int i = opened; i < quesIdx.size(); ++i) {
                str[quesIdx[i]] = ')';
            }
            std::swap(str[quesIdx[opened-1]], str[quesIdx[opened]]);
