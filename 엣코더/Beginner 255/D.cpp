...
int main() {
    IO io;
    auto [n, q] = io.scans<Long, 2>();
    auto arr = io.scanLongs(n, 1);
    auto queries = io.scanLongs(q);
    ...
    auto sumRange = [&](int left, int right) {
        return sum[right] - sum[left-1];
    };

    for (Long q : queries) {
        if (q <= arr[1]) {
            io.println(sum.back() - q*n);
        } else if (arr.back() <= q) {
            io.println(q*n - sum.back());
        } else {
            int lower = std::lower_bound(arr.begin()+1, arr.end(), q) - arr.begin();
            Long left = q*(lower-1) - sumRange(1, lower-1);
            Long right = sumRange(lower, n) - q*(n-lower+1);
