...
int main() {
    int n = io.nextInt();
    auto value = io.nextLongs(n);

    std::vector<std::vector<std::pair<Long, Long>>> dist(n, std::vector<std::pair<Long, Long>>(n, std::make_pair(INT64_MAX/2, INT64_MAX/2)));
    
    for (int i = 0; i < n; ++i) {
        auto line = io.nextToken();
        for (int j = 0; j < n; ++j) {
            if (line[j] == 'Y') {
                dist[i][j] = { 1, -value[j] };
            }
    ...
    for (int k = 0; k < n; ++k) {
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                dist[i][j] = std::min(dist[i][j], { dist[i][k].first + dist[k][j].first, dist[i][k].second + dist[k][j].second });
            }
        }
    }
    ...
    while (q--) {
        ...
        if (dist[u-1][v-1].first == INT64_MAX/2) {
            io.println("Impossible");
        } else {
            io.println(dist[u-1][v-1].first, -(-value[u-1] + dist[u-1][v-1].second));
        }
    }
