...
fn main() {
    ...
    let mut acc_by_val: BTreeMap<usize, Vec<usize>> = BTreeMap::new();
    for i in 0..n {
        acc_by_val
            .entry(arr[i])
    ...
    let max_by_val = acc_by_val
        .iter()
        .map(|(val, accs)| *accs.last().unwrap() - *accs.first().unwrap() + *val)
        .collect::<Vec<_>>();
    let max = max_by_val.iter().max().copied().unwrap();
    let count = max_by_val.iter().filter(|m| **m == max).count();
    writeln!(io.writer, "{max} {count}").unwrap();
}
