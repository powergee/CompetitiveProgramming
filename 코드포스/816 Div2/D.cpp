...
int main() {
    ...
    Tensor<2, int> bits = createTensor(-1, n, 31);
    using BitPos = std::pair<int, int>;
    std::vector<std::pair<BitPos, BitPos>> posOr, negOr;
    for (int qi = 0; qi < q; ++qi) {
        auto [i, j, x] = io.nexts<int, 3>();
        if (i > j) {
            std::swap(i, j);
        }
        for (int e = 0; e < 30; ++e) {
            ...
            BitPos pj = { j-1, e };
            if (x & (1<<e)) {
                posOr.emplace_back(pi, pj);
            } else {
                negOr.emplace_back(pi, pj);
            }
        }
    }

    for (auto [pi, pj] : negOr) {
        bits[pi.first][pi.second] = 0;
        bits[pj.first][pj.second] = 0;
    }

    for (auto [pi, pj] : posOr) {
        if (pi.first == pj.first) {
            bits[pi.first][pi.second] = 1;
        }
    }

    std::sort(posOr.begin(), posOr.end(), [&](const std::pair<BitPos, BitPos>& a, const std::pair<BitPos, BitPos>& b) {
        return std::make_pair(a.first.first, a.second.first) < std::make_pair(b.first.first, b.second.first);
    });

    for (auto [pi, pj] : posOr) {
        if (bits[pi.first][pi.second] == 1 || bits[pj.first][pj.second] == 1) {
            continue;
        } else if (bits[pj.first][pj.second] == 0) {
            bits[pi.first][pi.second] = 1;
        } else if (bits[pi.first][pi.second] == 0) {
            bits[pj.first][pj.second] = 1;
        }
    }
    ...
    for (auto [pi, pj] : posOr) {
        if (bits[pi.first][pi.second] == 1 || bits[pj.first][pj.second] == 1) {
            continue;
        } else if (bits[pj.first][pj.second] == -1) {
            bits[pj.first][pj.second] = 1;
        } else if (bits[pi.first][pi.second] == -1) {
            bits[pi.first][pi.second] = 1;
        } else {
            assert(false);
        }
