...
int main() {
    ...
    using Card = std::vector<int>;
    std::vector<Card> cards(n);
    for (int i = 0; i < n; ++i) {
        cards[i] = io.nextInts(k);
    }
    std::set<Card> cardSet(cards.begin(), cards.end());
    std::set<Card> metaCardSet;
    std::map<Card, int> setCount;

    for (int i = 0; i < n; ++i) {
        for (int j = i+1; j < n; ++j) {
            Card need(k);
            for (int ci = 0; ci < k; ++ci) {
                if (cards[i][ci] == cards[j][ci]) {
                    need[ci] = cards[i][ci];
                } else if (cards[i][ci] != 0 && cards[j][ci] != 0) {
                    need[ci] = 0;
                } else if (cards[i][ci] != 1 && cards[j][ci] != 1) {
                    need[ci] = 1;
                } else if (cards[i][ci] != 2 && cards[j][ci] != 2) {
                    need[ci] = 2;
                }
            }
            if (cardSet.contains(need)) {
                std::vector<Card> metaSet { need, cards[i], cards[j] };
                std::sort(metaSet.begin(), metaSet.end());
                Card flatted;
                flatted.reserve(3*k);
                for (auto& c : metaSet) {
                    for (int v : c) {
                ...
                if (!metaCardSet.contains(flatted)) {
                    metaCardSet.insert(flatted);
                    setCount[need]++;
                    setCount[cards[i]]++;
                    setCount[cards[j]]++;
                }
