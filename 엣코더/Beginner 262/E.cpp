...
int main() {
    ...
    auto degrees = countElements(map(graph, [](auto& adj) { return adj.size() % 2; }));
    degrees[0]--;

    Long answer = 0;
    for (int oddCnt = 0; oddCnt <= k; oddCnt += 2) {
        if (oddCnt <= degrees[1] && k-oddCnt <= degrees[0]) {
            answer += getComb(degrees[1], oddCnt) * getComb(degrees[0], k-oddCnt);
            answer %= MOD;
        }
    }
    io.println(answer);
