...
int main() {
    ...
    std::vector<int> isLuckyInit(SIZE, 1);
    for (int i = 0; i < SIZE; i += 2) {
        isLuckyInit[i] = 0;
    }
    SegTree<int> isLuckyTree(isLuckyInit, std::plus<int>());
    for (int i = 3; i < SIZE; i += 2) {
        if (isLuckyTree.query(i, i)) {
            std::vector<int> toUpdate;
            for (int j = i; j < SIZE; j += i) {
                if (isLuckyTree.query(0, SIZE-1) < j) {
                    break;
                }
                toUpdate.push_back(isLuckyTree.findLowerBound(j));
            }
            for (int u : toUpdate) {
                isLuckyTree.update(u, -1);
            }
        }
    }
    ...
    while (T--) {
        int n;
        scanf("%d", &n);
        printf(isLuckyTree.query(n, n) ? "lucky\n" : "unlucky\n");
    }
