...
int main() {
    ...
    auto r = io.nextMatrix(k, n);
    auto m = io.nextMatrix(k, n);

    Memo getMaxSatis(-1LL, k+1, 9, 9, 9, 9, 9);
    getMaxSatis = [&](int day, int c1, int c2, int c3, int c4, int c5) -> Long {
        if (day == k) {
            return 0;
        }

        Long result = 0;
        int cans[5] = { c1, c2, c3, c4, c5 };
        for (int i = 0; i < 5; ++i) {
            for (int j = 0; j < 5; ++j) {
                Long curr = r[day][i] + m[day][j];
                cans[i]--;
                cans[j]--;
    ...
    a.resize(5, 0);
    io.println(getMaxSatis(0, a[0], a[1], a[2], a[3], a[4]));
