...
int main() {
    ...
    auto headProfit = io.nextLongs(n);
    std::vector<Long> bonus(n+1, 0);
    for (int i = 0; i < m; ++i) {
        auto [c, y] = io.nexts<Long, 2>();
        bonus[c] = y;
    }

    Tensor<2, Long> dp = createTensor(-1LL, n, n+2);
    std::function<Long(int, int)> getMaxProfit = [&](int th, int counter) -> Long {
        Long currBonus = bonus[counter];
        if (th == n) {
            return currBonus;
        }

        Long& result = dp[th][counter];
        if (result != -1) {
            return result;
        }
        
        // Tail
        result = currBonus + getMaxProfit(th+1, 0);
        // Head
        result = std::max(result, currBonus + headProfit[th] + getMaxProfit(th+1, std::min(counter+1, n+1)));

        return result;
    };

    io.println(getMaxProfit(0, 0));
