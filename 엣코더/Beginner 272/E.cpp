...
int main() {
    ...
    std::vector<std::set<int>> existingTimes(n);
    for (int i = 0; i < n; ++i) {
        // arr[i] + (t+1)*(i+1) >= 0
        // t+1 >= -arr[i] / (i+1)
        // t >= -(arr[i]/(i+1)) - 1
        for (int t = std::max(0, -(arr[i]/(i+1)) - 1); t < m; ++t) {
            int val = arr[i] + (t+1)*(i+1);
            if (val >= n) {
                break;
            }
            if (val >= 0) {
                existingTimes[val].insert(t);
            }
        }
    }

    std::vector<int> answer(m, 0);
    std::set<int> window = existingTimes[0];
    for (int t : window) {
        answer[t] = 1;
    }
    for (int v = 1; v < n; ++v) {
