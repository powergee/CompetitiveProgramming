...
int main() {
    io.codeforces([&]() {
        int n = io.nextInt();
        auto types = io.nextInts(n);
        auto damages = io.nextLongs(n);

        std::vector<Long> buckets[2];
        for (int i = 0; i < n; ++i) {
            buckets[types[i]].push_back(damages[i]);
        }

        Long answer = 0;
        if (buckets[0].empty()) {
            answer = std::accumulate(buckets[1].begin(), buckets[1].end(), 0LL);
        } else if (buckets[1].empty()) {
            answer = std::accumulate(buckets[0].begin(), buckets[0].end(), 0LL);
        } else if (buckets[0].size() == buckets[1].size()) {
            answer = std::accumulate(damages.begin(), damages.end(), 0LL)*2 - *std::min_element(damages.begin(), damages.end());
        } else {
            auto smaller = (buckets[0].size() < buckets[1].size() ? buckets[0] : buckets[1]);
            auto bigger = (buckets[0].size() < buckets[1].size() ? buckets[1] : buckets[0]);
            std::sort(bigger.begin(), bigger.end(), std::greater<>());
            answer = std::accumulate(smaller.begin(), smaller.end(), 0LL)*2 +
                     std::accumulate(bigger.begin(), bigger.begin()+smaller.size(), 0LL) + 
                     std::accumulate(bigger.begin(), bigger.end(), 0LL);
        }
