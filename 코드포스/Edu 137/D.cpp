...
int main() {
    int n = io.nextInt();
    auto bin = io.nextToken();
    auto start = bin.find('1');
    if (start == bin.npos) {
        io.println("0");
        return 0;
    }
    ...
    int gap = 0;
    for (int i = start; i+1 < n;) {
        auto nextOne = bin.find('1', i+1);
        if (nextOne == bin.npos) {
            gap = std::max(gap, n-i);
            i = n;
        } else {
            gap = std::max(gap, (int)nextOne-i);
            i = nextOne;
        }
    }

    std::string answer = bin.substr(start);
    for (int g = 1; g <= gap; ++g) {
        std::string curr = bin;
        for (int i = start; i < n; ++i) {
            if (i-g >= 0) {
                curr[i] = bin[i-g] == '1' ? '1' : curr[i];
            }
        }
