...
bool determineNaively(std::vector<int> arr) {
    std::set<int> arrSet(arr.begin(), arr.end());
    int n = arr.size();
    for (int i = 0; i < n; ++i) {
        for (int j = i+1; j < n; ++j) {
            for (int k = j+1; k < n; ++k) {
                if (arrSet.count(arr[i] + arr[j] + arr[k]) == 0) {
                    return false;
                }
            }
        }
...
int main() {
    io.codeforces([&]() {
        ...
        auto isClosure = [&]() -> bool {
            if (arr.size() <= 6) {
                return determineNaively(arr);
            }

            int leftNegCount = 0, rightPosCount = 0;
            for (int i = 0; i < 3; ++i) {
                if (arr[i] < 0) {
                    ++leftNegCount;
                }
            ...
            if (leftNegCount == 3 || rightPosCount == 3) {
                return false;
            }
            ...
            return determineNaively(compressed);
        };

        io.printYes(isClosure());
    });
