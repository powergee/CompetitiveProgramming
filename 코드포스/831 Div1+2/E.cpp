...
int getLIS(std::vector<int>& arr) {
    ...
    for (int i = 0; i < arr.size(); ++i) {
        auto found = std::upper_bound(lis.begin(), lis.end(), arr[i]);
        *found = arr[i];
...
int main() {
    ...
    std::function<int(int)> initHeight = [&](int start) -> int {
        for (int next : tree[start]) {
            int subHeight = initHeight(next);
            height[start] = std::max(height[start], subHeight+1);
        }
        return height[start];
    };
    initHeight(1);
    ...
    std::vector<int> seq;
    int avail = 0;
    std::function<int(int)> appendSeq = [&](int start) -> int {
        std::vector<std::pair<int, int>> nextHeight;
        nextHeight.reserve(tree[start].size());
        for (int next : tree[start]) {
            nextHeight.emplace_back(next, height[next]);
        }
        std::sort(nextHeight.begin(), nextHeight.end(), [](auto p1, auto p2) {
            return p1.second > p2.second;
        });

        int curr = INT32_MAX;
        if (nextHeight.size()) {
            for (auto [next, h] : nextHeight) {
                curr = std::min(curr, appendSeq(next));
            }
        } else {
            curr = ++avail;
        }
