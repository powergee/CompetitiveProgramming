...
fn main() {
    let mut io = IO::stdio();
    let n = io.usize();
    let mut moves = (0..n).map(|_| (io.usize(), io.usize())).collect::<Vec<_>>();
    moves.sort_by(|&(au, ad), &(bu, bd)| {
        let a_dif = au as isize - ad as isize;
        let b_dif = bu as isize - bd as isize;
        b_dif.cmp(&a_dif)
    });

    let mut meteo = Vec::with_capacity(n);
    let mut height = 0isize;
    for (up, down) in moves {
        height += up as isize;
        meteo.push(height);
        height -= down as isize;
    }
    let bottom = height.min(0);
    let mut answer = 0;
    for m in meteo {
        answer += m - bottom;
    }
