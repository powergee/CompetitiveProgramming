...
int main() {
    io.codeforces([&]() {
        auto t = io.nextToken();
        int n = io.nextInt();
        auto s = io.nextTokens(n);

        std::vector<std::pair<int, int>> maxLength(t.size(), std::make_pair(0, -1));
        for (int i = 0; i < t.size(); ++i) {
            for (int j = 0; j < n; ++j) {
                if (i+1 >= s[j].size() && t.substr(i+1-s[j].size(), s[j].size()) == s[j] && maxLength[i].first < (int)s[j].size()) {
                    maxLength[i] = { (int)s[j].size(), j };
                }
            }
        }
        ...
        auto isPossible = [&]() {
            if (maxLength.back().first == 0) {
                return false;
            }

            int remain = int(t.size())-maxLength.back().first;
            answer.emplace_back(maxLength.back().second+1, int(t.size())-maxLength.back().first+1);
            while (remain > 0) {
                // io.println(remain);
                int min = remain;
                std::pair<int, int> minPos = { -1, -1 };
                for (int i = remain-1; i < t.size(); ++i) {
                    if (maxLength[i].first == 0) {
                        continue;
                    }
                    int curr = i+1-maxLength[i].first;
                    // io.println("i", i, "curr", curr);
                    if (curr < min) {
                        min = curr;
                        minPos = { maxLength[i].second+1, i-maxLength[i].first+2 };
                    }
                }
                if (min == remain) {
                    return false;
