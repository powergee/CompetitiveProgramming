...
int main() {
    io.codeforces([&]() {
        ...
        auto s = io.nextLongs(k);

        io.printYes([&]() {
            if (k == 1) {
                return true;
            }
            std::vector<Long> restored(k, -1);
            for (int i = k-1; i > 0; --i) {
                restored[i] = s[i] - s[i-1];
            }
            // n-1-(k-2) = n-1-k+2 = n-k+1;
            if (!std::is_sorted(restored.begin()+1, restored.end())) {
                return false;
            }
            return (s[0] <= (n-k+1)*restored[1]);
        }());
    });
