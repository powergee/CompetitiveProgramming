...
int main() {
    int n = io.nextInt();
    Tensor<2, std::pair<Long, Long>> nums(n);
    std::map<Long, Long> globalLCM;
    std::map<Long, int> headCount;
    for (int i = 0; i < n; ++i) {
        int m = io.nextInt();
        nums[i].resize(m);
        for (int j = 0; j < m; ++j) {
            auto [p, e] = io.nexts<Long, 2>();
            nums[i][j] = { p, e };
            if (globalLCM[p] == e) {
                headCount[p]++;
            } else if (globalLCM[p] < e) {
                headCount[p] = 1;
                globalLCM[p] = e;
            }
        }
    ...
    int remainSame = 0, change = 0;
    for (int i = 0; i < n; ++i) {
        bool found = false;
        for (int j = 0; j < nums[i].size(); ++j) {
            auto [p, e] = nums[i][j];
            if (globalLCM[p] == e && headCount[p] == 1) {
                found = true;
                break;
            }
        }

        (found ? change : remainSame)++;
    }

    io.println(change + (remainSame ? 1 : 0));
