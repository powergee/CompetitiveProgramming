...
int main() {
    auto [n, m, l] = io.nexts<int, 3>();
    auto map = io.nextTokens(n);
    auto target = io.nextToken();

    std::map<char, int> tCnt = countElements(target);
    std::map<char, int> mCnt;
    std::deque<std::pair<int, int>> pos[26];
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < m; ++j) {
            pos[map[i][j] - 'a'].emplace_back(i, j);
            mCnt[map[i][j]]++;
        }
    }

    int ableToMake = INT32_MAX;
    for (auto [c, cnt] : tCnt) {
        ableToMake = std::min(ableToMake, mCnt[c] / cnt);
    }
    
    std::string path;
    auto appendPath = [&](int sr, int sc, int er, int ec) {
        if (sr < er) {
            for (int r = sr; r < er; ++r) {
                path += 'D';
            }
    ...
    int currR = 0, currC = 0;
    for (int i = 0; i < ableToMake; ++i) {
        for (char c : target) {
            auto next = pos[c-'a'].front();
            pos[c-'a'].pop_front();
            appendPath(currR, currC, next.first, next.second);
            path += 'P';
            currR = next.first;
            currC = next.second;
        }
    }
    appendPath(currR, currC, n-1, m-1);

    io.println(ableToMake, path.size());
    io.println(path);
