...
int main() {
    ...
    auto s = io.nextToken();
    int sOriginSize = s.size();
    s.resize(sOriginSize+TMAX);

    std::vector<int> answer(sOriginSize+TMAX, 0);
    int i = 1, j = 0;
    for (; i < sOriginSize; ++i) {
        while (j > 0 && s[i] != s[j]) {
            j = answer[j-1];
    ...
    Memo transJ(-1, 26, sOriginSize);
    transJ = [&](int si, int j) -> int {
        if (j > 0 && char('a' + si) != s[j]) {
            return transJ(si, answer[j-1]);
        } else {
            return j;
        }
    };
    ...
    while (q--) {
        ...
        for (int i = sOriginSize; i < sOriginSize+t.size(); ++i) {
            io.print(answer[i]);
        }
        io.println();
        
        std::fill(answer.begin()+sOriginSize, answer.end(), 0);
        i = pi;
        j = pj;
    }
