...
int main() {
    auto [h1, w1] = io.nexts<int, 2>();
    auto m1 = io.nextMatrix(h1, w1);
    auto [h2, w2] = io.nexts<int, 2>();
    auto m2 = io.nextMatrix(h2, w2);

    bool found = false;
    for (int mask = 0; mask < (1<<w1); ++mask) {
        if (__builtin_popcount(mask) != w2) {
            continue;
        }
        int r2 = 0;
        for (int r1 = 0; r1 < h1; ++r1) {
            ...
            for (int c1 = 0; c1 < w1; ++c1) {
                if ((mask&(1<<c1)) && c2 < w2 && m1[r1][c1] == m2[r2][c2]) {
                    c2++;
                }
            }
            if (c2 == w2) {
