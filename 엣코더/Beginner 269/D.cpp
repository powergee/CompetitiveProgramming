...
int main() {
    ...
    int count = 0;
    for (int i = 0; i < n; ++i) {
        if (visited.count(black[i]) == 0) {
            count++;
            std::queue<std::pair<int, int>> q;
            visited.insert(black[i]);
            q.push(black[i]);
            ...
            while (!q.empty()) {
                ...
                for (int i = 0; i < 6; ++i) {
                    ...
                    auto np = std::make_pair(nx, ny);
                    if (visited.count(np) == 0 && blackSet.count(np)) {
                        visited.insert(np);
                        q.push(np);
                    }
                }
