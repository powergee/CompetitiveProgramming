...
int main() {
    io.codeforces([&]() {
        int n = io.nextInt();
        Tensor<2, std::string> words(3);
        for (int i = 0; i < 3; ++i) {
            words[i] = io.nextTokens(n);
        }

        std::map<std::string, std::vector<int>> owners;
        for (int j = 0; j < n; ++j) {
            for (int i = 0; i < 3; ++i) {
                owners[words[i][j]].push_back(i);
            }
        }

        std::vector<int> answer(3, 0);
        for (auto& soPair : owners) {
            if (soPair.second.size() == 1) {
                answer[soPair.second[0]] += 3;
            } else if (soPair.second.size() == 2) {
                answer[soPair.second[0]]++;
                answer[soPair.second[1]]++;
            }
        }
