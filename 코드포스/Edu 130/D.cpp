...
int main() {
    ...
    auto queryCurrent = [&](int pos) -> char {
        io.println("? 1", pos+1);
        std::cout.flush();
        auto res = io.scanToken();
        return res[0];
    };

    auto queryRange = [&](int left, int right) -> int {
        static std::map<std::pair<int, int>, int> cache;
        auto bucket = std::make_pair(left, right);
        if (cache.count(bucket)) {
            return cache[bucket];
        }

        io.println("? 2", left+1, right+1);
        std::cout.flush();
        auto res = io.scanInt();
        return cache[bucket] = res;
    };

    std::string result(n, '@');
    result[0] = queryCurrent(0);
    std::vector<std::pair<char, int>> distinct { { result[0], 0 } };

    for (int i = 1; i < n; ++i) {
        if (queryRange(0, i) > distinct.size()) {
            result[i] = queryCurrent(i);
            distinct.emplace_back(result[i], i);
        } else {
            auto isValid = [&](int mid) -> bool {
                int res = queryRange(distinct[mid].second, i);
                return res == distinct.size() - mid;
            };

            int left = 0, right = distinct.size();
            while (left < right) {
            ...
            result[i] = distinct[left-1].first;
            distinct[left-1].second = i;
            std::sort(distinct.begin(), distinct.end(), [](auto d1, auto d2) { return d1.second < d2.second; });
        }
    }
