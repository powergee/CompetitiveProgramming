...
int main() {
    int n = io.nextInt();
    auto arr = io.nextTokens(n);

    auto lengths = map(arr, [](std::string s) { return s.size(); });
    std::sort(lengths.begin(), lengths.end(), std::greater<>());
    
    int targetLength = lengths[0] + lengths[1] + lengths[2];
    std::vector<std::vector<std::string>> strByLength(lengths[0]+1);
    for (auto& str : arr) {
        strByLength[str.size()].push_back(str);
    }
    for (int i = 0; i < strByLength.size(); ++i) {
        std::sort(strByLength[i].begin(), strByLength[i].end(), [](const std::string& s1, const std::string& s2) {
    ...
    Long answer = 0;
    for (int i = 0; i < strByLength.size(); ++i) {
        for (int j = i; j < strByLength.size(); ++j) {
            for (int k = j; k < strByLength.size(); ++k) {
                int need[strByLength.size()];
                std::fill(need, need+strByLength.size(), 0);
                need[i]++;
                need[j]++;
                need[k]++;
                if (strByLength[i].size() < need[i] || strByLength[j].size() < need[j] || strByLength[k].size() < need[k]) {
                    continue;
                }
                ...
                for (int i = 0; i < strByLength.size(); ++i) {
                    for (int j = 0; j < need[i]; ++j) {
                        cands.push_back(strByLength[i][j]);
                    }
                }
                ...
                do {
                    ...
                    answer = std::max(std::stoll(concat), answer);
