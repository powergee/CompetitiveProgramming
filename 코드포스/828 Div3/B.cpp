...
int main() {
    io.codeforces([&]() {
        ...
        std::pair<Long, Long> even = { 0, 0 }, odd = { 0, 0 };
        for (auto val : io.nextLongs(n)) {
            auto& p = (val % 2 == 0 ? even : odd);
            p.first += val;
            p.second++;
        }

        while (q--) {
            auto [op, x] = io.nexts<Long, 2>();
            if (op == 0) {
                // add to even
                if (x % 2 == 0) {
                    even.first += x * even.second;
                } else {
                    odd.second += even.second;
                    odd.first += x * even.second + even.first;
                    even = { 0, 0 };
                }
            } else {
                // add to odd
                if (x % 2 == 0) {
                    odd.first += x * odd.second;
                } else {
                    even.second += odd.second;
                    even.first += x * odd.second + odd.first;
                    odd = { 0, 0 };
                }
            }
            io.println(even.first + odd.first);
        }
    });
