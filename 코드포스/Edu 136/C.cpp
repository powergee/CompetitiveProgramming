...
int main() {
    io.codeforces([&]() {
        int n = io.nextInt();
        using Count = std::tuple<Long, Long, Long>;
        
        std::vector<Count> dp(n+1);
        dp[0] = { 0, 0, 1 };
        for (int cards = 2; cards <= n; cards += 2) {
            auto [nw, nl, nd] = dp[cards-2];
            Long win = getComb(cards-1, (cards-1)/2) + nl;
            Long lose = 0;
            if (cards >= 4) {
                lose = getComb(cards-2, (cards-4)/2) + nw;
            }
            Long draw = nd;
            
            dp[cards] = { win%MOD, lose%MOD, draw%MOD };
        }

        auto [win, lose, draw] = dp.back();
        io.println(win, lose, draw);
    });
