...
int main() {
    ...
    std::set<std::pair<int, std::vector<int>*>> faceup;
    for (int i = 0; i < n; ++i) {
        auto found = faceup.lower_bound(std::make_pair(arr[i], (std::vector<int>*)0));
        if (found == faceup.end()) {
            if (k == 1) {
                answer[arr[i]-1] = i+1;
            } else {
                ...
                bucket->push_back(i);
                faceup.insert(std::make_pair(arr[i], bucket));
            }
        } else {
            std::vector<int>* bucket = found->second;
            bucket->push_back(i);
            faceup.erase(found);
            if (bucket->size() == k) {
                for (int index : (*bucket)) {
                    answer[arr[index]-1] = i+1;
                }
            } else {
                faceup.insert(std::make_pair(arr[i], bucket));
            }
