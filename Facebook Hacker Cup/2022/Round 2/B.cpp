...
using PQType = std::priority_queue<BigInt, std::vector<BigInt>, std::greater<BigInt>>;
const BigInt MOD = 1'000'000'007LL;

int main() {
    io.google([&]() {
        auto [n, k] = io.nexts<int, 2>();
        std::vector<std::tuple<int, int, BigInt, BigInt>> people(n);
        std::map<int, std::vector<std::pair<int, BigInt>>> buy;
        std::map<int, std::vector<BigInt>> buyPrices;
        for (int i = 0; i < n; ++i) {
            auto [a, b, x, y] = io.nexts<int, 4>();
            people[i] = { a, b, x, y };
            buy[a].emplace_back(i, x);
            buyPrices[a].push_back(x);
        }
        for (auto& [i, _] : buy) {
            std::sort(buy[i].begin(), buy[i].end(), [](auto p, auto q) {
                return p.second < q.second;
            });
            std::sort(buyPrices[i].begin(), buyPrices[i].end());
        }
        
        std::vector<bool> visited(n+1, false);
        std::vector<std::vector<BigInt>> canMakeFrom(n+1);
        std::map<int, std::vector<std::vector<BigInt>>> preAcc;

        auto toVector = [&](PQType q) {
        ...
        std::function<void(int)> searchProfits = [&](int start) {
            if (visited[start]) {
                return;
            }
            visited[start] = true;
            auto [a, b, x, y] = people[start];
            ...
            if (preAcc.find(b) == preAcc.end()) {
                preAcc[b] = std::vector<std::vector<BigInt>>(buy[b].size());
                PQType q;
                for (int i = std::ssize(buy[b])-1; i >= 0; --i) {
                    auto [next, price] = buy[b][i];
                    searchProfits(next);
                    for (BigInt nextPro : canMakeFrom[next]) {
                        q.push(price + nextPro);
                        while (q.size() > k) {
                            q.pop();
                        }
                    }
                    preAcc[b][i] = toVector(q);
                }
            }

            auto it = std::upper_bound(buyPrices[b].begin(), buyPrices[b].end(), y);
            PQType q;
            q.push(0);
            if (it != buyPrices[b].end()) {
                int index = it - buyPrices[b].begin();
                for (BigInt v : preAcc[b][index]) {
                    q.push(v - y);
                    while (q.size() > k) {
                        q.pop();
                    }
            ...
            canMakeFrom[start] = toVector(q);
        };

        PQType q;
        for (int i = 0; i < n; ++i) {
            searchProfits(i);
            for (auto pro : canMakeFrom[i]) {
                q.push(pro);
                while (q.size() > k) {
                    q.pop();
                }
