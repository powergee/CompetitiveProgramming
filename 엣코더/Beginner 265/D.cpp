...
int main() {
    ...
    io.printYes([&]() {
        for (int i = 1; i <= n; ++i) {
            auto jIt = std::lower_bound(sum.begin(), sum.end(), sum[i-1]+p);
            if (jIt == sum.end() || *jIt != sum[i-1]+p) {
                continue;
            }
            int j = jIt - sum.begin() + 1;

            auto kIt = std::lower_bound(sum.begin(), sum.end(), sum[j-1]+q);
            if (kIt == sum.end() || *kIt != sum[j-1]+q) {
                continue;
            }
            int k = kIt - sum.begin() + 1;

            auto lIt = std::lower_bound(sum.begin(), sum.end(), sum[k-1]+r);
            if (lIt == sum.end() || *lIt != sum[k-1]+r) {
                continue;
            }
            int l = lIt - sum.begin() + 1;
            return true;
        }
        return false;
    }());
