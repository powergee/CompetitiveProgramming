...
int main() {
    io.google([&]() {
        ...
        auto board = io.nextTokens(n);

        Tensor<2, int> dist = createTensor(INT32_MAX/2, n, m);
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                if (board[i][j] == '1') {
                    dist[i][j] = 0;
        ...
        auto isPossible = [&](int maxDist) -> bool {
            std::vector<int> posDiff, negDiff;
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < m; ++j) {
                    if (dist[i][j] > maxDist) {
                        posDiff.push_back(i+j);
                        negDiff.push_back(i-j);
                    }
                }
            }
            if (posDiff.empty()) {
            ...
            std::sort(posDiff.begin(), posDiff.end());
            std::sort(negDiff.begin(), negDiff.end());
            
            for (int i = 0; i < n; ++i) {
                for (int j = 0; j < m; ++j) {
                    int currPos = i+j;
                    int currNeg = i-j;
                    int posMax = std::max(std::abs(posDiff.front()-currPos), std::abs(posDiff.back()-currPos));
                    int negMax = std::max(std::abs(negDiff.front()-currNeg), std::abs(negDiff.back()-currNeg));
                    int currDist = std::max(posMax, negMax);
                    if (currDist <= maxDist) {
                        return true;
                    }
                }
