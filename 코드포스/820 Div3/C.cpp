...
int main() {
    io.codeforces([&]() {
        auto tiles = io.nextToken();
        int cost = std::abs(tiles.back() - tiles.front());

        std::vector<int> indexes[26];
        for (int i = 0; i < tiles.size(); ++i) {
            indexes[tiles[i]-'a'].push_back(i);
        }
        int diff = (tiles.front() <= tiles.back() ? 1 : -1);

        std::vector<int> answer;
        answer.reserve(tiles.size());
        for (int i = tiles.front()-'a'; ; i += diff) {
