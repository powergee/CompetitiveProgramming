...
int main() {
    io.codeforces([&]() {
        ...
        auto path = io.nextInts(n);
        
        std::vector<std::pair<int, int>> queries(k);
        std::vector<const char*> answer(k, "NO");
        std::map<int, std::vector<int>> pending, traveling;

        for (int i = 0; i < k; ++i) {
            auto [src, dst] = io.nexts<int, 2>();
            queries[i] = {src, dst};
            pending[src].push_back(i);
        }

        for (int pos : path) {
            for (int i : traveling[pos]) {
                answer[i] = "YES";
            }
            traveling[pos].clear();
            for (int i : pending[pos]) {
                traveling[queries[i].second].push_back(i);
            }
            pending[pos].clear();
        }
