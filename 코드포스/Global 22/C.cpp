...
int main() {
    io.codeforces([&]() {
        ...
        int even = 0, odd = 0;
        for (int i = 0; i < n; ++i) {
            (arr[i] % 2 == 0 ? even : odd) += 1;
        }
        Long total = std::accumulate(arr.begin(), arr.end(), 0LL);

        int want[2] = { 1, ((total % 2 == 0) ? 0 : 1) };
        Memo canWin(-1, even+1, odd+1, 2, 2, 2);
        canWin = [&](int evenRem, int oddRem, int currIsEven, int oppoIsEven, int currSide) -> int {
            if (evenRem == 0 && oddRem == 0) {
                return want[currSide] == currIsEven ? 1 : 0;
            }
            if (evenRem && !canWin(evenRem-1, oddRem, oppoIsEven, currIsEven, !currSide)) {
                return 1;
            }
            if (oddRem && !canWin(evenRem, oddRem-1, oppoIsEven, !currIsEven, !currSide)) {
                return 1;
            }
            return 0;
        };

        io.println(canWin(even, odd, 1, 1, 0) ? "Alice" : "Bob");
    });
