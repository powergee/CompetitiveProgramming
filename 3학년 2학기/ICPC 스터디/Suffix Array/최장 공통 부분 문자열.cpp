...
int main() {
    ...
    std::cin >> a >> b;
    std::string comb = a + "$" + b;
    ManberMyers manber(comb);

    auto& sa = manber.getSuffixArray();
    auto& lcp = manber.getLCP();
    
    int maxLength = -1;
    int maxIdx = -1;
    for (int i = 1; i < manber.getLength()-1; ++i) {
        bool prev = (sa[i] < a.size() ? true : false);
        bool next = (sa[i+1] < a.size() ? true : false);
        if (prev != next && maxLength < lcp[i]) {
            maxLength = lcp[i];
            maxIdx = i;
        }
    }
    ...
    std::cout << maxLength << "\n";
    int s1 = sa[maxIdx], s2 = sa[maxIdx+1];
    while (s1 < comb.size() && s2 < comb.size() && comb[s1] == comb[s2]) {
        std::cout << comb[s1];
        ++s1; ++s2;
    }
