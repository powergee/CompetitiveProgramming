...
int main() {
    ...
    auto isPossible = [&](Long targetMin) -> bool {
        Long supply = 0;
        for (int i = 0; i < n; ++i) {
            if (arr[i] > targetMin) {
                supply += (arr[i]-targetMin) / b;
            }
        }
        for (int i = 0; i < n; ++i) {
            Long currValue = arr[i];
            if (currValue < targetMin) {
                Long need = targetMin-currValue;
                Long cost = need / a + ((need % a) ? 1 : 0);
                if (supply < cost) {
                    return false;
                }
                supply -= cost;
