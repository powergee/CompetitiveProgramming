...
int main() {
    using Cand = std::tuple<int, int, int>;

    auto [n, x, y, z] = io.nexts<int, 4>();
    auto a = io.nextInts(n);
    auto b = io.nextInts(n);

    std::vector<Cand> cands;
    std::vector<int> accepted;
    for (int i = 0; i < n; ++i) {
        cands.emplace_back(a[i], b[i], i+1);
    }

    auto getRight = [](std::vector<Cand>& cands, int start) {
    ...
    std::sort(cands.begin(), cands.end(), [](Cand c1, Cand c2) {
        auto [a1, b1, i1] = c1;
        auto [a2, b2, i2] = c2;
        if (a1 != a2) {
            return a1 > a2;
    ...
    for (int i = 0; i < x; ++i) {
        accepted.push_back(std::get<2>(cands[i]));
    }
    cands = getRight(cands, x);
