...
int main() {
    ...
    std::vector<bool> visited(n+1, false), inStack(n+1, false), inCycle(n+1, false);
    ...
    std::function<bool(int, int)> findCycle = [&](int start, int parent) -> bool {
        ...
        for (int next : graph[start]) {
            if (next == parent) {
                ...
                return true;
            } else if (!visited[next] && findCycle(next, start)) {
                return true;
            }
        }
    ...
    std::vector<int> subTreeBelong(n+1, -1);
    std::function<void(int, int)> findSubTree = [&](int start, int belong) {
        subTreeBelong[start] = belong;
        if (inCycle[start]) {
            for (int next : graph[start]) {
                if (subTreeBelong[next] == -1 && !inCycle[next]) {
                    findSubTree(next, belong);
                }
            }
        } else {
            for (int next : graph[start]) {
                if (subTreeBelong[next] == -1) {
                    findSubTree(next, belong);
                }
            }
        }
    ...
    int bId = 1;
    for (int i = 1; i <= n; ++i) {
        if (subTreeBelong[i] == -1 && !inCycle[i]) {
            findSubTree(i, bId++);
        }
    }
    ...
    while (q--) {
        ...
        if (inCycle[u] && inCycle[v]) {
            io.printYes(false);
        } else if (subTreeBelong[u] != subTreeBelong[v]) {
            io.printYes(false);
        } else {
            io.printYes(true);
        }
