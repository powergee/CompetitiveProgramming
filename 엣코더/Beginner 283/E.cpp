...
int main() {
    ...
    auto mat = io.nextMatrix(h, w);

    const int UND = -2;
    const int IMPOSSIBLE = h+2;

    Memo getMinCost(UND, h+1, 2, 2);
    getMinCost = [&](int row, int upperChanged, int currChange) -> int {
        if (row == h) {
            return 0;
        }
        ...
        // do not flip next
        {
            ...
            for (int c = 0; c < w; ++c) {
                bool hasSame = false;
                // left
                hasSame |= c-1 >= 0 && mat[row][c] == mat[row][c-1];
                // right
                hasSame |= c+1 < w && mat[row][c] == mat[row][c+1];
                // upper
                hasSame |= row-1 >= 0 && (mat[row][c] ^ currChange) == (mat[row-1][c] ^ upperChanged);
                // lower
                hasSame |= row+1 < h && (mat[row][c] ^ currChange) == mat[row+1][c];
            ...
            if (valid) {
                result = std::min(result, getMinCost(row+1, currChange, 0));
            }
        }
        // flip next
        {
            ...
            if (valid) {
                result = std::min(result, 1+getMinCost(row+1, currChange, 1));
            }
        }
        return result;
    };

    int answer = std::min(getMinCost(0, 0, 0), 1+getMinCost(0, 0, 1));
