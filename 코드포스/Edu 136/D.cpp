...
int main() {
    io.codeforces([&]() {
        ...
        auto parents = io.nextInts(n-1, 2);
        std::vector<std::vector<int>> tree(n+1);
        for (int v = 2; v <= n; ++v) {
            tree[parents[v]].push_back(v);
        }
        std::set<int> firstChild(tree[1].begin(), tree[1].end());

        auto countRequired = [&](int maxHeight) -> int {
            int answer = 0;
            std::function<int(int)> getHeight = [&](int start) {
                int height = 0;
                for (int next : tree[start]) {
                    int sub = getHeight(next);
                    height = std::max(sub+1, height);
                }
                if (height == maxHeight-1 && start != 1 && firstChild.count(start) == 0) {
                    answer += 1;
                    return -1;
                } else {
                    return height;
