...
struct Trie {
    ...
    void insert(const std::string& str, int index = 0) {
        count++;
        if (str.size() <= index) {
            hasLeaf = true;
            return;
        }
        int nextIdx = str[index] == '.';
        if (next[nextIdx] == nullptr) {
            next[nextIdx] = new Trie;
        }
        next[nextIdx]->insert(str, index+1);
    }

    void findEmptyEntry(std::string& result, int index, int size) {
        assert(!hasLeaf);
        if (index == size) {
            assert(count == 0);
            return;
        ...
        assert(!next[0]->hasLeaf || !next[1]->hasLeaf);
        if (!next[0]->hasLeaf && (next[1]->hasLeaf || next[0]->count <= next[1]->count)) {
            result[index] = '-';
            next[0]->findEmptyEntry(result, index+1, size);
        } else {
            result[index] = '.';
            next[1]->findEmptyEntry(result, index+1, size);
        }
    }

    std::string findEmptyEntry(int size) {
        std::string result;
        result.resize(size, '.');
        findEmptyEntry(result, 0, size);
        return result;
    }
};
...
int main() {
    io.google([&]() {
        ...
        Tensor<2, std::pair<int, Long>> graph(n+1);
        for (int i = 0; i < m; ++i) {
            auto [u, v, c] = io.nexts<int, 3>();
            graph[u].emplace_back(v, c);
            graph[v].emplace_back(u, c);
        }

        const int EDGE_BOUND = int(std::sqrt(m)) + 1;
        Tensor<2, Long> flowMemo(n+1);
        for (int u = 1; u <= n; ++u) {
            std::sort(graph[u].begin(), graph[u].end());
            if (graph[u].size() >= EDGE_BOUND) {
                flowMemo[u].resize(n+1, 0);
                for (auto [v, c1] : graph[u]) {
                    for (auto [w, c2] : graph[v]) {
                        flowMemo[u][w] += std::min(c1, c2);
                    }
                }
            }
        }
        ...
        while (q--) {
            ...
            Long result = 0;
            auto it = std::lower_bound(graph[u].begin(), graph[u].end(), std::make_pair(v, 0LL));
            ...
            if (graph[u].size() >= EDGE_BOUND) {
                result += flowMemo[u][v];
            } else if (graph[v].size() >= EDGE_BOUND) {
                result += flowMemo[v][u];
            } else {
                std::map<int, Long> bridge;
                for (auto [w, c] : graph[u]) {
                    bridge[w] = c;
                }
                for (auto [w, c] : graph[v]) {
                    result += std::min(bridge[w], c);
                }
            }
