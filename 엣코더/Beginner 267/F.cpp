...
int main() {
    auto n = io.nextInt();
    auto tree = io.nextGraph(n, n-1);

    auto findFarthest = [&](int start) -> int {
    ...
    int dia1 = findFarthest(1);
    int dia2 = findFarthest(dia1);

    int q = io.nextInt();
    Tensor<2, std::pair<int, int>> queries(n+1);
    for (int i = 0; i < q; ++i) {
        auto [u, k] = io.nexts<int, 2>();
        queries[u].emplace_back(k, i);
    }

    for (int i = 1; i <= n; ++i) {
        std::sort(queries[i].begin(), queries[i].end(), std::greater<>());
    }

    std::vector<int> answer(q, -1);
    for (int root : { dia1, dia2 }) {
        std::vector<int> stack;
        stack.reserve(n+1);
        std::function<void(int)> iterateCandidates = [&](int start) {
            while (queries[start].size() && queries[start].back().first <= stack.size()) {
                answer[queries[start].back().second] = stack[stack.size() - queries[start].back().first];
                queries[start].pop_back();
            }
            stack.push_back(start);

            for (int next : tree[start]) {
                if (stack.size() <= 1 || stack[stack.size()-2] != next) {
                    iterateCandidates(next);
                }
            }
            stack.pop_back();
        };
        iterateCandidates(root);
    }
