...
int main() {
    ...
    auto pivots = io.scanInts(k);
    std::set<int> pivotSet(pivots.begin(), pivots.end());

    std::vector<std::pair<int, int>> light, dark;

    for (int i = 1; i <= n; ++i) {
        auto [x, y] = io.scans<int, 2>();
        if (pivotSet.count(i)) {
            light.emplace_back(x, y);
        } else {
            dark.emplace_back(x, y);
        }
    }
    ...
    for (auto [dx, dy] : dark) {
        ...
        for (auto [lx, ly] : light) {
            minDist = std::min(minDist, std::sqrt(std::pow(lx-dx, 2) + std::pow(ly-dy, 2)));
        }
        answer = std::max(answer, minDist);
    }
