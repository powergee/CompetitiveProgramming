...
int main() {
    ...
    auto heights = io.nextLongs(n);

    std::vector<Long> goLeft(n, 0), goRight(n, 0);
    for (int i = 0; i+1 < n; ++i) {
        goLeft[i] = std::max(heights[i+1]-heights[i], 0LL);
    }
    for (int i = 1; i < n; ++i) {
        goRight[i] = std::max(heights[i-1]-heights[i], 0LL);
    }

    auto getSumVector = [&](std::vector<Long>& arr) {
    ...
    auto goLeftSum = getSumVector(goLeft);
    auto goRightSum = getSumVector(goRight);

    auto getSum = [&](std::vector<Long>& sum, int left, int right) {
        if (left == 0) {
            return sum[right];
        }
    ...
    while (m--) {
        auto [s, t] = io.nexts<int, 2>();
        s--, t--;
        if (s < t) {
            io.println(getSum(goRightSum, s+1, t));
        } else {
            io.println(getSum(goLeftSum, t, s-1));
        }
    }
