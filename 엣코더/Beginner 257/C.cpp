...
int main() {
    int n = io.scanInt();
    auto kind = io.scanToken();
    auto weights = io.scanInts(n);

    int childCount = 0, adultCount = 0;
    for (char ch : kind) {
        (ch == '1' ? adultCount : childCount)++;
    }

    std::vector<std::pair<int, char>> people(n);
    for (int i = 0; i < n; ++i) {
        people[i] = { weights[i], kind[i] };
    }
    std::sort(people.begin(), people.end(), [](auto p, auto q) { 
        if (p.first != q.first) {
    ...
    int answer = 0;
    int prevChild = 0, prevAdult = 0;
    for (int i = 0; i < n; ++i) {
        int correct = prevChild + (adultCount - prevAdult);
        answer = std::max(answer, correct);
        (people[i].second == '1' ? prevAdult : prevChild)++;
    }
    answer = std::max(answer, prevChild);
    io.println(answer);
