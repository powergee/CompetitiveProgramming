...
int main() {
    io.codeforces([&]() {
        ...
        Long answer = INT32_MAX;
        // 간선 수가 홀수인거 하나 지우기
        for (int i = 1; i <= n; ++i) {
            if (graph[i].size() % 2 == 1) {
                answer = std::min(answer, unhappy[i]);
            }
        }
        // 이웃한, 간선수가 짝수인거 한 쌍 지우기
        for (auto [u, v] : edges) {
            if (graph[u].size() % 2 == 0 && graph[v].size() % 2 == 0) {
                answer = std::min(answer, unhappy[u] + unhappy[v]);
            }
        }
        io.println(answer);
    });
