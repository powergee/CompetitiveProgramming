...
int main() {
    io.codeforces([&]() {
        ...
        for (int i = 0; i < n; ++i) {
            points[i].first = io.nextDouble();
        }
        for (int i = 0; i < n; ++i) {
            points[i].second = io.nextDouble();
        }

        std::sort(points.begin(), points.end());
        
        double currPos = points.front().first;
        double currHeight = points.front().second;

        for (int i = 1; i < n; ++i) {
            auto [pos, height] = points[i];
            if (currPos == pos) {
                if (currHeight < height) {
                    currHeight = height;
                }
            } else {
                double slope = (height-currHeight) / (pos-currPos);
                if (slope > 1) {
                    currPos = pos;
                    currHeight = height;
                } else if (slope > -1) {
                    double y = (pos-currPos + currHeight+height) / 2;
                    double x = y - currHeight + currPos;
                    currPos = x;
                    currHeight = y;
                }
            }
