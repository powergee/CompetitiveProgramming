...
int main() {
    ...
    auto arr = io.nextLongs(m);
    int q = io.nextInt();

    SegTree<Long> maxTree(arr, [](Long a, Long b) { return std::max(a, b); });

    while (q--) {
        auto [sx, sy, ex, ey, k] = io.nexts<Long, 5>();
        io.printYes([&]() -> bool {
            if (std::abs(sx-ex) % k != 0 || std::abs(sy-ey) % k != 0) {
                return false;
            }
            Long maxHeight = (n-sx)/k*k + sx;
            Long leftCol = std::min(sy, ey);
            Long rightCol = std::max(sy, ey);
            if (maxTree.query(leftCol-1, rightCol-1) >= maxHeight) {
                return false;
            }
            return true;
        }());
