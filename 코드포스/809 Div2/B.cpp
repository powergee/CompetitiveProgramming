...
int main() {
    io.codeforces([&]() {
        ...
        std::vector<int> maxHeight(n+1, 0), currHeight(n+1, 0), lastAppeared(n+1, -1);
        for (int i = 0; i < n; ++i) {
            int dist = i-lastAppeared[arr[i]];
            if (lastAppeared[arr[i]] == -1) {
                currHeight[arr[i]] = 1;
            ...
            maxHeight[arr[i]] = std::max(maxHeight[arr[i]], currHeight[arr[i]]);
            
        }

        for (int i = 1; i <= n; ++i) {
            io.print(maxHeight[i]);
        }
        io.println();
    });
