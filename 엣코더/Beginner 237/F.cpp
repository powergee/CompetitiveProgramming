...
int main() {
    ...
    Memo countSequences(-1LL, n+1, m+1, m+1, m+1);
    countSequences = [&](int curr, int lis1, int lis2, int lis3) -> Long {
        if (curr == 0) {
            return (lis1 && lis2 && lis3) ? 1 : 0;
        }
        Long result = 0;
        for (int v = m; v >= 1; --v) {
            if (v >= lis1) {
                result += countSequences(curr-1, v, lis2, lis3);
            } else if (v >= lis2) {
                result += countSequences(curr-1, lis1, v, lis3);
            } else if (v >= lis3) {
                result += countSequences(curr-1, lis1, lis2, v);
            }
        }
        return result % MOD;
    };

    io.println(countSequences(n, 0, 0, 0));
