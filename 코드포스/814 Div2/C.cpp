...
int main() {
    io.codeforces([&]() {
        ...
        using Athlete = std::pair<int, int>;
        std::vector<Athlete> line(n);
        for (int i = 0; i < n; ++i) {
            line[i].first = io.nextInt();
            line[i].second = i+1;
        }

        Athlete front = line[0];
        std::deque<Athlete> spare(line.begin()+1, line.end());
        
        using Query = std::tuple<int, int, int>;
        std::vector<Query> queries(q);
        for (int i = 0; i < q; ++i) {
            auto [id, rounds] = io.nexts<int, 2>();
            queries[i] = { id, rounds, i };
        }
        std::sort(queries.begin(), queries.end(), [](auto p1, auto p2) {
            auto [id1, round1, i1] = p1;
            auto [id2, round2, i2] = p2;
            return round1 < round2;
        });

        std::vector<int> winCount(n+1, 0);
        std::vector<int> answer(q);
        int currRound = 0, currQuery = 0;
        while (currQuery < q && front.first != n) {
            auto next = spare.front();
            spare.pop_front();

            if (front.first < next.first) {
                spare.push_back(front);
                front = next;
                winCount[next.second]++;
            } else {
                spare.push_back(next);
                winCount[front.second]++;
            }

            currRound++;
            while (currQuery < q && std::get<1>(queries[currQuery]) == currRound) {
                auto [id, rounds, i] = queries[currQuery];
                answer[i] = winCount[id];
                ++currQuery;
            }
        }
        ...
        if (currQuery < q) {
            ...
            position[front.second] = -1;
            for (int i = 0; i < n-1; ++i) {
                position[spare.front().second] = i;
                spare.pop_front();
            }

            for (int i = currQuery; i < q; ++i) {
                auto [id, rounds, qi] = queries[i];
                if (id == front.second) {
                    answer[qi] = winCount[id] + (rounds - currRound);
                } else {
                    answer[qi] = winCount[id];
                }
            }
        }
