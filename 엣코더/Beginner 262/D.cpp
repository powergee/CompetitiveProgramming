...
int main() {
    ...
    Long answer = 0;
    for (int k = 1; k <= n; ++k) {
        Tensor<3, Long> dp = createTensor(0LL, n+1, k+1, k);
        for (int remaining = 0; remaining <= k; ++remaining) {
            for (int sum = 0; sum < k; ++sum) {
                dp[n][remaining][sum] = ((remaining == 0 && sum == 0) ? 1 : 0);
            }
        }
        for (int start = n-1; start >= 0; --start) {
            for (int remaining = 0; remaining <= k; ++remaining) {
                for (int sum = 0; sum < k; ++sum) {
                    if (remaining == 0) {
                        dp[start][remaining][sum] = (sum == 0 ? 1 : 0);
                    } else if (n-start < remaining) {
                        dp[start][remaining][sum] = 0;
                    } else {
                        dp[start][remaining][sum] = (dp[start+1][remaining][sum] + dp[start+1][remaining-1][(sum+arr[start]) % k]) % MOD;
                    }
                }
            }
        }
