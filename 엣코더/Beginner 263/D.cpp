...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        sum += arr[i];
        answer = std::min(answer, total - (sum - l*(i+1)));
    }
    sum = 0;
    for (int i = n-1; i >= 0; --i) {
        sum += arr[i];
        answer = std::min(answer, total - (sum - r*(n-i)));
    }
    for (int i = 0; i < n; ++i) {
        answer = std::min(answer, l*(i+1) + r*(n-i-1));
    }

    std::vector<Long> rightProfit(n, 0);
    std::multiset<Long, std::greater<Long>> considering;
    sum = 0;
    for (int i = n-1; i >= 0; --i) {
        sum += arr[i];
        if (r*(n-i) < sum) {
            rightProfit[i] = sum - r*(n-i);
            considering.insert(sum - r*(n-i));
        }
    }
    ...
    for (int i = 0; i+1 < n; ++i) {
        if (rightProfit[i]) {
            considering.erase(considering.find(rightProfit[i]));
        }

        sum += arr[i];
        if (considering.size()) {
            answer = std::min(answer, total - std::max(0LL, sum - l*(i+1)) - *considering.begin());
        } else {
            answer = std::min(answer, total - std::max(0LL, sum - l*(i+1)));
        }
    }
