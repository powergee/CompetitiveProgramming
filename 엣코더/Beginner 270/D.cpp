...
int main() {
    ...
    auto arr = io.nextLongs(k);
    
    Memo getMaxProfit(-1LL, n+1);
    getMaxProfit = [&](int stones) -> Long {
        if (stones == 0) {
            return 0;
        }
        Long result = INT64_MIN;
        for (int i = 0; i < arr.size() && arr[i] <= stones; ++i) {
            result = std::max(result, arr[i]-getMaxProfit(stones-arr[i]));
        }
        return result;
    };

    Long profit = getMaxProfit(n);
    // x+y = n
    // x-y = profit
    // 2*x = n+profit
    io.println((n+profit)/2);
