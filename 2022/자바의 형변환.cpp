...
int main() {
    int n = io.nextInt();
    std::map<std::string, int> strSet;
    Tensor<2, int> tree(n);
    for (int i = 0; i < n-1; ++i) {
        auto [child, parent] = io.nexts<std::string, 2>();
        int cid, pid;
        if (strSet.count(child)) {
            cid = strSet[child];
        } else {
            cid = strSet.size();
            strSet[child] = cid;
        }
        if (strSet.count(parent)) {
            pid = strSet[parent];
        } else {
            pid = strSet.size();
            strSet[parent] = pid;
    ...
    auto [uStr, vStr] = io.nexts<std::string, 2>();
    int u = strSet[uStr];
    int v = strSet[vStr];

    auto isReachable = [&](int src, int dst) -> bool {
        std::queue<int> q;
        std::vector<bool> visited(strSet.size(), false);
        visited[src] = true;
        q.push(src);
    ...
    io.println((isReachable(u, v) || isReachable(v, u)) ? 1 : 0);
