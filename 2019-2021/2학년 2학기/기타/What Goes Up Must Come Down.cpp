...
int main()
{
    ...
    Node* root = buildNode(0, n-1, cards);
    std::vector<std::pair<int, int>> cardIdxPairs(n);
    for (int i = 0; i < n; ++i)
        cardIdxPairs[i] = {cards[i], i};
    
    std::sort(cardIdxPairs.begin(), cardIdxPairs.end());
    long long result = 0;
    for (int i = 0; i < n; ++i)
    {
        int left = countBigger(root, cardIdxPairs[i].first, 0, cardIdxPairs[i].second-1);
        int right = countBigger(root, cardIdxPairs[i].first, cardIdxPairs[i].second+1, n-1);
