...
int main() {
    int n = io.nextInt();
    auto x = io.nextInts(n, 1);
    auto c = io.nextLongs(n, 1);

    Tensor<2, std::pair<int, int>> graph(n+1);
    for (int i = 1; i <= n; ++i) {
        graph[i].emplace_back(x[i], c[i]);
    }
    ...
    std::function<void(int)> findCycle = [&](int start) {
        ...
        for (auto [next, weight] : graph[start]) {
            if (inStack[next]) {
                int add = weight;
                for (auto it = stack.rbegin(); it->first != next; ++it) {
                    add = std::min(add, it->second);
                }
                answer += add;
            } else if (!visited[next]) {
                stack.emplace_back(next, weight);
                findCycle(next);
            }
        }
        inStack[start] = false;
    ...
    for (int i = 1; i <= n; ++i) {
        if (!visited[i]) {
            stack.emplace_back(i, INT32_MAX);
            findCycle(i);
        }
    }
    io.println(answer);
