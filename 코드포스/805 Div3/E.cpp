...
int main() {
    io.codeforces([&]() {
        int n = io.nextInt();
        std::vector<std::pair<int, int>> domino;
        domino.reserve(n);
        bool possible = true;
        std::vector<int> appeared(n+1, 0);
        Tensor<2, int> existingIntervals(n+1);

        for (int i = 0; i < n; ++i) {
            auto [a, b] = io.nexts<int, 2>();
            if (a > b) {
                std::swap(a, b);
            }
            
            // basic checks
            if (a == b || appeared[a] >= 2 || appeared[b] >= 2) {
                possible = false;
            }
            domino.emplace_back(a, b);
            appeared[a]++;
            appeared[b]++;
            existingIntervals[a].push_back(i);
            existingIntervals[b].push_back(i);
        }
        ...
        Tensor<2, int> graph(n);
        for (int i = 1; i <= n; ++i) {
            for (int j = 0; j < existingIntervals[i].size(); ++j) {
                for (int k = j+1; k < existingIntervals[i].size(); ++k) {
                    int u = existingIntervals[i][j];
                    int v = existingIntervals[i][k];
                    graph[u].push_back(v);
                    graph[v].push_back(u);
                }
            }
        ...
        std::function<bool(int, int)> paintColor = [&](int start, int baseColor) -> bool {
            color[start] = baseColor;
            for (int next : graph[start]) {
                if (color[next] == -1) {
                    bool subResult = paintColor(next, baseColor == 1 ? 0 : 1);
                    if (!subResult) {
                        return false;
                    }
                } else if (color[next] == baseColor) {
                    return false;
                }
            }
            return true;
