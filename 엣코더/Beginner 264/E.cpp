...
int main() {
    ...
    std::vector<bool> alive(e, true);
    int q = io.nextInt();
    std::vector<int> queries(q);
    for (int i = 0; i < q; ++i) {
        queries[i] = io.nextInt()-1;
        alive[queries[i]] = false;
    }
    ...
    for (int i = 0; i < e; ++i) {
        if (alive[i]) {
            int v1 = dsu.getCompValue(edges[i].first);
            int v2 = dsu.getCompValue(edges[i].second);
            if (v1 == 0 && v2 > 0) {
                currElec += dsu.getCompSize(edges[i].first);
            } else if (v1 > 0 && v2 == 0) {
                currElec += dsu.getCompSize(edges[i].second);
            }
            dsu.unionNodes(edges[i].first, edges[i].second);
        }
    }
    ...
    for (int i = 0; i < queries.size(); ++i) {
        answer.push_back(currElec);

        int edge = queries[i];
        int v1 = dsu.getCompValue(edges[edge].first);
        int v2 = dsu.getCompValue(edges[edge].second);
        if (v1 == 0 && v2 > 0) {
            currElec += dsu.getCompSize(edges[edge].first);
        } else if (v1 > 0 && v2 == 0) {
            currElec += dsu.getCompSize(edges[edge].second);
        }
        dsu.unionNodes(edges[edge].first, edges[edge].second);
    }
