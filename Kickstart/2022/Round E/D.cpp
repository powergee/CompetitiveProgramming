...
int main() {
    io.google([&]() {
        auto [n, p, m, ar, ac] = io.nexts<int, 5>();
        --ar; --ac;

        const int dr[] { -1, 0, 0, 1 };
        const int dc[] { 0, 1, -1, 0 };
        std::pair<char, int> toll[4];

        for (int i = 0; i < 4; ++i) {
            std::string op = io.nextToken();
            int v = io.nextInt();
            toll[i] = { op[0], v };
        }

        auto applyOperation = [&](std::pair<char, int> opInfo, Long operand) {
            switch (opInfo.first)
            {
            case '+':
                return operand + opInfo.second;
            case '-':
                return operand - opInfo.second;
            case '*':
                return operand * opInfo.second;
            case '/':
                if (operand < 0) {
                    return operand / opInfo.second - ((std::abs(operand) % opInfo.second) ? 1 : 0);
                }
                return operand / opInfo.second;
            }
            return 0LL;
        ...
        std::map<std::pair<int, int>, std::pair<int, int>> deliver;
        for (int i = 0; i < p; ++i) {
            auto [x, y, c] = io.nexts<int, 3>();
            deliver[std::make_pair(x-1, y-1)] = { c, i };
        }
        
        const Long UND = INT64_MAX;
        const Long IMPOSSIBLE = INT64_MIN;
        Memo getMaxProfit(UND, n, n, m+1, (1<<p));
        getMaxProfit = [&](int r, int c, int time, int finished) -> Long {
            if (time == 0) {
                return (finished == ((1<<p)-1) && r == ar && c == ac) ? 0 : IMPOSSIBLE;
            }
            Long result = getMaxProfit(r, c, time-1, finished);
            for (int i = 0; i < 4; ++i) {
                int nr = r - dr[i];
                int nc = c - dc[i];
                if (0 <= nr && nr < n && 0 <= nc && nc < n) {
                    Long sub = getMaxProfit(nr, nc, time-1, finished);
                    if (sub != IMPOSSIBLE) {
                        result = std::max(result, applyOperation(toll[i], sub));
                    }
                    
                    if (deliver.count(std::make_pair(r, c))) {
                        auto delInfo = deliver[std::make_pair(r, c)];
                        if ((finished & (1<<delInfo.second)) == 0) {
                            sub = getMaxProfit(nr, nc, time-1, (finished | (1<<delInfo.second)));
                            if (sub != IMPOSSIBLE) {
                                result = std::max(result, applyOperation(toll[i], sub) + delInfo.first);
                            }
                        }
                    }
                }
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                answer = std::max(answer, getMaxProfit(i, j, m, 0));
            }
        }
