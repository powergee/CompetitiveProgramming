...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        auto [x, y, p] = io.scans<Long, 3>();
        points.emplace_back(x, y, p);
    }

    auto isPossible = [&](Long s) -> bool {
        auto getDist = [](Point p1, Point p2) {
            auto [x1, y1, _1] = p1;
            auto [x2, y2, _2] = p2;
        ...
        std::function<void(int)> DFS = [&](int start) {
            ...
            for (int next = 0; next < n; ++next) {
                if (!visited[next] && getDist(points[start], points[next]) <= s * std::get<2>(points[start])) {
                    DFS(next);
                }
            }
        };
    ...
    Long left = 1, right = 2*Long(INT32_MAX);
    while (left < right) {
