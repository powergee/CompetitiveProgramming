...
int main() {
    io.codeforces([&]() {
        ...
        auto arr = io.nextInts(m);

        auto jobs = countElements(arr);
        std::vector<std::pair<int, int>> jobVec;
        for (int i = 1; i <= n; ++i) {
            if (jobs.find(i) == jobs.end()) {
                jobVec.emplace_back(i, 0);
            } else {
                jobVec.emplace_back(i, jobs[i]);
            }
        }
        std::sort(jobVec.begin(), jobVec.end(), [](auto p1, auto p2) { return p1.second > p2.second; });
        
        auto isPossible = [&](int maxTime) -> bool {
            int unflushed = 0;
            for (auto [i, cnt] : jobVec) {
                int time = 0, currJobs = cnt;
                while ((currJobs || unflushed) && time < maxTime) {
                    if (currJobs) {
                        currJobs--;
                        time += 1;
                    } else if (time+2 <= maxTime && unflushed > 0) {
                        unflushed--;
                        time += 2;
                    } else {
                        break;
