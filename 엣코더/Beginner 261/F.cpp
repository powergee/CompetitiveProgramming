...
int main() {
    int n = io.nextInt();
    auto c = io.nextInts(n);
    auto x = io.nextInts(n);

    std::vector<OrderedSet<int>> originColors(n+1);
    std::vector<OrderedSet<int>> removedColors(n+1);
    OrderedSet<int> removed;
    std::vector<std::vector<std::pair<int, int>>> colorsByNumber(n+1);

    for (int i = 0; i < n; ++i) {
        originColors[c[i]].insert(i);
        colorsByNumber[x[i]].emplace_back(c[i], i);
    }

    Long answer = 0;
    for (int x = 1; x <= n; ++x) {
        for (auto [color, index] : colorsByNumber[x]) {
            auto it = originColors[color].lower_bound(index);
            int free = (it == originColors[color].end() ? originColors[color].size() : originColors[color].order_of_key(*it));

            it = removedColors[color].lower_bound(index);
            int fakeFree = (it == removedColors[color].end() ? removedColors[color].size() : removedColors[color].order_of_key(*it));

            it = removed.lower_bound(index);
            int removedCnt = (it == removed.end() ? removed.size() : removed.order_of_key(*it));

            answer += index - (free - fakeFree) - removedCnt;
            removedColors[color].insert(index);
