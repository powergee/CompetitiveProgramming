...
int main() {
    auto [s, t, m] = io.nexts<int, 3>();
    std::vector<std::vector<int>> graph(s+t+1);
    for (int i = 0; i < m; ++i) {
    ...
    Tensor<2, int> connected = createTensor(-1, t, t);
    for (int v = 1; v <= s; ++v) {
        for (int a : graph[v]) {
            for (int b : graph[v]) {
                if (a == b) { continue; }
                if (connected[a-s-1][b-s-1] == -1) {
                    connected[a-s-1][b-s-1] = v;
                } else {
                    io.println(v, a, b, connected[a-s-1][b-s-1]);
                    return 0;
                }
            }
        }
