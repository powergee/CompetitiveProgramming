...
int main() {
    ...
    io.codeforces([&]() {
        ...
        std::map<Long, std::vector<Long>> buckets;
        for (int i = 0; i < n; ++i) {
            buckets[arr[i]].push_back(i);
        }

        Long maxProfit = 1;
        std::tuple<Long, int, int> answer = { arr[0], 0, 0 };
        for (auto& [val, pos] : buckets) {
            if (pos.size() == 1) {
                continue;
            }
            std::vector<Long> profit(pos.size()-1, 0);
            for (int i = 0; i < pos.size()-1; ++i) {
                profit[i] = 1 - (pos[i+1]-pos[i]-1);
            }
            std::vector<Long> dp(pos.size()-1, 0);
            std::vector<std::pair<int, int>> range(pos.size()-1);
            dp[0] = profit[0];
            range[0] = { 0, 0 };

            for (int i = 1; i < pos.size()-1; ++i) {
                if (dp[i-1] + profit[i] < profit[i]) {
                    dp[i] = profit[i];
                    range[i] = { i, i };
                } else {
                    dp[i] = dp[i-1] + profit[i];
                    range[i] = { range[i-1].first, i };
                }
            }

            int maxIdx = std::max_element(dp.begin(), dp.end()) - dp.begin();
            Long finalProfit = dp[maxIdx]+1;
            if (maxProfit < finalProfit) {
                maxProfit = finalProfit;
                answer = { val, pos[range[maxIdx].first], pos[range[maxIdx].second+1] };
            }
        }

        auto [a, l, r] = answer;
        io.println(a, l+1, r+1);
    });
