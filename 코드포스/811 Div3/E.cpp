...
int main() {
    std::vector<int> init[10], loop[10];
    for (int s = 0; s < 10; ++s) {
        ...
        do {
            ...
            curr = (2*curr) % 10;
        } while (std::find(bucket.begin(), bucket.end(), bucket.back())-bucket.begin() == bucket.size()-1);
        int bound = std::find(bucket.begin(), bucket.end(), bucket.back())-bucket.begin();
        for (int i = 0; i < bucket.size()-1; ++i) {
            (i < bound ? init[s] : loop[s]).push_back(bucket[i]);
        }
    }

    io.codeforces([&]() {
        int n = io.nextInt();
        auto arr = io.nextInts(n);

        auto isPossibleSingle = [&](int start, int target) {
            if (target < start) {
                return false;
            }
            if (start == target) {
            ...
            if (loop[s].front() == 0 || curr > target) {
                return false;
            }
            curr += ((target-curr)/20*20);
            int li = 0;
            while (curr < target) {
                curr += loop[s][li++];
            }
        ...
        auto isPossible = [&](int v) {
            for (int i = 0; i < n; ++i) {
                if (!isPossibleSingle(arr[i], v)) {
                    return false;
                }
            }
            return true;
        ...
        io.printYes([&]() {
            auto it = std::find_if(arr.begin(), arr.end(), [](int v) { return v % 10 == 0; });
            if (it != arr.end()) {
                return isPossible(*it);
            }
            it = std::find_if(arr.begin(), arr.end(), [](int v) { return v % 10 == 5; });
            if (it != arr.end()) {
                return isPossible(*it) || isPossible(*it+5);
            }
