...
fn main() {
    let mut io = IO::stdio();
    let (n, m, mut p) = (io.usize(), io.usize(), io.usize());
    let succ = 'result: {
        for _ in 0..n {
            let mut items = 0;
            let mut enemies = vec![];
            for _ in 0..m {
                ...
                match floor {
                    -1 => items += 1,
                    _ => enemies.push(floor as usize),
                }
            }
            enemies.sort_by(|a, b| b.cmp(a));

            while let Some(weakest) = enemies.pop() {
                while items > 0 && p < weakest {
                    items -= 1;
                    p *= 2;
                }
                if p < weakest {
    ...
    writeln!(io.writer, "{}", if succ { 1 } else { 0 }).unwrap();
}
