...
int main() {
    ...
    while (m--) {
        auto [i, x] = io.nexts<Long, 2>();
        --i;
        Long prevGain1 = 0, prevGain2 = 0;
        ...
        Long diff1 = currGain1-prevGain1;
        Long diff2 = currGain2-prevGain2;
        answer += diff1 * (i*(n-i));
        answer += diff2 * ((i+1)*(n-i-1));
        io.println(answer);
    } 
