...
int main() {
    ...
    Memo isPossible(-1, n, x+1);
    isPossible = [&](int cid, int goal) -> int {
        if (cid == 0) {
            return goal % coins[0].first == 0 && goal / coins[0].first <= coins[0].second;
        }
        for (int i = 0; i <= coins[cid].second; ++i) {
            if (i*coins[cid].first > goal) {
                break;
            }
            if (isPossible(cid-1, goal - i*coins[cid].first)) {
                return true;
            }
        }
        return false;
