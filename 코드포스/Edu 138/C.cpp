...
int main() {
    io.codeforces([&]() {
        ...
        int answer = 0;
        for (int k = 1; k <= n; ++k) {
            std::multiset<int, std::greater<int>> avail(arr.begin(), arr.end());
            bool success = true;
            for (int stage = 1; stage <= k; ++stage) {
                int max = k-stage+1;
                auto found = avail.lower_bound(max);
                if (found == avail.end()) {
                    success = false;
                    break;
                }
                avail.erase(found);
                if (avail.size()) {
                    int base = *std::prev(avail.end());
                    avail.erase(std::prev(avail.end()));
                    avail.insert(base+max);
                }
            }
            if (success) {
                answer = k;
