...
int main() {
    io.codeforces([&]() {
        int n = io.nextInt();
        auto lids = io.nextToken();
        auto values = io.nextInts(n);

        Memo getMaxSaved(-1LL, n+1, 2, 2);
        getMaxSaved = [&](int start, int prevSaved, int currSaved) -> Long {
            if (start == n) {
                return 0;
            }
            if (lids[start] == '0') {
                return getMaxSaved(start+1, currSaved, 0);
            }
            Long answer = 0;
            if (!prevSaved && start > 0) {
                answer = std::max(answer, values[start-1] + getMaxSaved(start+1, currSaved, 0));
            }
            if (!currSaved) {
                answer = std::max(answer, values[start] + getMaxSaved(start+1, 1, 0));
            }
            return answer;
        };

        io.println(getMaxSaved(0, 0, 0));
    });
