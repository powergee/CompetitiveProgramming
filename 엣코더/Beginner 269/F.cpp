...
const Long INV2 = getModInverse(2);

Long getSumOfArith(Long init, Long step, Long count) {
    return (count * ((2*init%MOD) + ((count-1)*step%MOD)) % MOD) * INV2 % MOD;
}
...
int main() {
    ...
    int q = io.nextInt();
    while (q--) {
        auto [a, b, c, d] = io.nexts<Long, 4>();

        auto getSumOfRow = [&](int p, int c, int d) {
            if ((p+c) % 2) {
                return getSumOfArith(m*(p-1)+c+1, 2, (d-c+1)/2);
            } else {
                return getSumOfArith(m*(p-1)+c, 2, (d-c+1)/2 + ((d-c+1)%2));
            }
        };

        Long answer = 0;
        if (a == b) {
            answer = getSumOfRow(a, c, d);
        } else {
            Long odd = getSumOfRow(a, c, d);
            Long even = getSumOfRow(a+1, c, d);

            Long oddCount = (b-a+1)/2 + (b-a+1)%2;
            Long evenCount = b-a+1 - oddCount;

            Long oddDiff, evenDiff;
            if ((a+c) % 2) {
                oddDiff = ((d-c+1)/2) * 2*m % MOD;
                evenDiff = ((d-c+1)/2 + ((d-c+1)%2)) * 2*m % MOD;
            } else {
                oddDiff = ((d-c+1)/2 + ((d-c+1)%2)) * 2*m % MOD;
                evenDiff = ((d-c+1)/2) * 2*m % MOD;
            }

            answer = getSumOfArith(odd, oddDiff, oddCount) + getSumOfArith(even, evenDiff, evenCount);
            answer %= MOD;
        }
        io.println(answer);
    }
