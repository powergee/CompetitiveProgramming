...
int main() {
    ...
    int maxTime = 0;
    std::vector<std::pair<int, int>> timetable(100001, {-1, -1});
    for (int i = 0; i < n; ++i) {
        auto [t, x, a] = io.nexts<int, 3>();
        timetable[t] = { x, a };
        maxTime = std::max(maxTime, t);
    }
    timetable.resize(maxTime+1);

    Memo getMaxProfit(-1LL, 5, timetable.size());
    getMaxProfit = [&](int pos, int time) -> Long {
        Long result = timetable[time].first == pos ? timetable[time].second : 0;
        Long sub = 0;
        for (int next = 0; next <= 4; ++next) {
            int dist = std::abs(next-pos);
            int nextTime = time + std::max(dist, 1);
            if (nextTime < timetable.size()) {
                sub = std::max(sub, getMaxProfit(next, nextTime));
            }
        }
        return result + sub;
    };
