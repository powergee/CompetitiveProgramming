...
fn main() {
    let mut io = IO::stdio();
    let n = io.usize();
    let mut trees = io.usizes(n);
    trees.sort_unstable();
    let mut interval = trees[1] - trees[0];
    for i in 1..n - 1 {
        interval = gcd(interval, trees[i + 1] - trees[i]);
    }

    let mut need = 0;
    for i in 0..n - 1 {
        need += (trees[i + 1] - trees[i]) / interval - 1;
    }
    writeln!(io.writer, "{need}").unwrap();
}
