...
fn main() {
    let mut io = IO::stdio();
    let n = io.usize();
    let words = io.strings(n);

    let mut digits = (0..10)
        .map(|i| (b'A' + i, false, 0usize))
        .collect::<Vec<_>>();
    for word in words {
        digits[(word.as_bytes().first().unwrap() - b'A') as usize].1 = true;
        let mut mult = 1;
        for c in word.bytes().rev() {
            digits[(c - b'A') as usize].2 += mult;
            mult *= 10;
        }
    }

    digits.sort_by_key(|&(_, _, count)| count);
    for i in 0..10 {
        if !digits[i].1 {
            digits.remove(i);
            break;
    ...
    writeln!(
        ...
        digits
            .into_iter()
            .enumerate()
            .fold(0, |sum, (i, (_, _, count))| sum + (i + 1) * count)
    )
    .unwrap();
}
