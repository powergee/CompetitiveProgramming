...
int main() {
    io.codeforces([&]() {
        ...
        for (int add = 1; add <= maxVal; ++add) {
            std::vector<int> home(maxVal+add+1, 0), away(maxVal+add+1, 0);
            for (auto [v, cnt] : count) {
                home[v] += cnt;
                away[v+add] += cnt;
            }
            int dp[maxVal+add+1][3];
            for (int last = 0; last <= maxVal+add; ++last) {
                for (int homeReduce = 0; homeReduce <= std::min(home[last], 2); ++homeReduce) {
                    int currHome = home[last]-homeReduce;
                    int& result = dp[last][homeReduce];
                    if (last >= add) {
                        result = dp[last-add][0] + (currHome >= 2 ? currHome : 0);
                        if (away[last] >= 1 && currHome+1 >= 2) {
                            result = std::max(result, dp[last-add][1] + currHome+1);
                        }
                        if (away[last] >= 2) {
                            result = std::max(result, dp[last-add][2] + currHome+2);
                        }
                    } else {
                        result = (currHome >= 2 ? currHome : 0);
                    }
                }
            }
            int subAnswer = 0;
            for (int i = maxVal+1; i <= maxVal+add; ++i) {
                subAnswer += dp[i][0];
            }
            answer = std::max(answer, subAnswer);
        }
        io.println(answer);
    });
