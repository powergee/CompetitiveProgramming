...
int main()
{
    ...
    if (able)
    {
        ...
        for (int i = 0; i <= n - 1; ++i)
            vertex.emplace_back(belong[i], i);
        for (int i = Not(0); i <= Not(n - 1); ++i)
            vertex.emplace_back(belong[i], i);

        std::sort(vertex.begin(), vertex.end(), std::greater<std::pair<int, int>>());

        for (auto v : vertex)
        {
            printf("{%d, %d}\n", v.first, v.second);
            if (v.second >= MAXINPUT && result[Not(v.second)] == -1)
                result[Not(v.second)] = 1;
            else if (result[v.second] == -1)
                result[v.second] = 0;
        }
