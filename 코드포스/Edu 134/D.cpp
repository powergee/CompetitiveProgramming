...
int main() {
    io.codeforces([&]() {
        ...
        std::vector<std::vector<int>> aSet(1), bSet(1);
        aSet[0] = io.nextInts(n);
        bSet[0] = io.nextInts(n);

        int answer = 0;
        for (int e = 29; e >= 0; --e) {
            assert(aSet.size() == bSet.size());
            bool valid = true;
            std::vector<std::vector<int>> aSetNext, bSetNext;
            aSetNext.reserve(aSet.size()*2);
            bSetNext.reserve(bSet.size()*2);
            for (int s = 0; s < aSet.size(); ++s) {
                assert(aSet[s].size() == bSet[s].size());
                int aOne = 0;
                int bZero = 0;
                for (int i = 0; i < aSet[s].size(); ++i) {
                    if ((aSet[s][i] & (1<<e)) > 0) {
                        aOne++;
                    }
                    if ((bSet[s][i] & (1<<e)) == 0) {
                ...
                if (aOne == aSet[s].size()) {
                    aSetNext.emplace_back(aSet[s]);
                    bSetNext.emplace_back(bSet[s]);
                } else {
                    aSetNext.emplace_back();
                    aSetNext.emplace_back();
                    bSetNext.emplace_back();
                    bSetNext.emplace_back();

                    int aZeroIdx, aOneIdx, bZeroIdx, bOneIdx;
                    aZeroIdx = bOneIdx = aSetNext.size()-2;
                    aOneIdx = bZeroIdx = aSetNext.size()-1;

                    for (int i = 0; i < aSet[s].size(); ++i) {
                        if ((aSet[s][i] & (1<<e))) {
                            aSetNext[aOneIdx].push_back(aSet[s][i]);
                        } else {
                            aSetNext[aZeroIdx].push_back(aSet[s][i]);
                        }
                        if ((bSet[s][i] & (1<<e))) {
                            bSetNext[bOneIdx].push_back(bSet[s][i]);
                        } else {
                            bSetNext[bZeroIdx].push_back(bSet[s][i]);
                        }
                    }
                }
            }
