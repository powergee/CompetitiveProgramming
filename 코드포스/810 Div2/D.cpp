...
int main() {
    io.codeforces([&]() {
        ...
        std::vector<std::pair<Long, Long>> rains;
        rains.reserve(n);
        std::vector<std::pair<Long, int>> pivots;
        pivots.reserve(3*n);

        for (int i = 0; i < n; ++i) {
            auto [x, p] = io.nexts<Long, 2>();
            rains.emplace_back(x, p);
            pivots.emplace_back(x-p, 1); // beginning
            pivots.emplace_back(x, -2); // middle
            pivots.emplace_back(x+p, 1); // ending
        }

        std::sort(pivots.begin(), pivots.end());
        
        Long slope = 0, currValue = 0;
        Long leftReq = INT64_MIN, rightReq = INT64_MIN;
        for (int i = 0; i+1 < 3*n; ++i) {
            slope += pivots[i].second;
            currValue += (pivots[i+1].first - pivots[i].first) * slope;
            
            if (currValue > m) {
                leftReq = std::max(leftReq, (currValue-m)-pivots[i+1].first);
                rightReq = std::max(rightReq, (currValue-m)+pivots[i+1].first);
            }
        }
        ...
        answer.reserve(n+1);
        for (int i = 0; i < n; ++i) {
            if (rains[i].second-rains[i].first >= leftReq && rains[i].second+rains[i].first >= rightReq) {
                answer += '1';
            } else {
                answer += '0';
            }
