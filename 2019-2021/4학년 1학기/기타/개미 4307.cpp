...
int main() {
    io.codeforces([&]() {
        auto [l, n] = io.nexts<int, 2>();
        auto pos = io.nextInts(n);
        std::sort(pos.begin(), pos.end());

        int min = INT32_MIN, max = INT32_MIN;
        for (int i = 0; i < n; ++i) {
            min = std::max(min, std::min(pos[i], l-pos[i]));
            max = std::max(max, std::max(pos[i], l-pos[i]));
        }
