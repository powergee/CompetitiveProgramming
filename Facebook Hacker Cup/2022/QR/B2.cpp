...
int main() {
    io.google([&]() {
        ...
        auto isInGrid = [&](int r, int c) {
            return 0 <= r && r < n && 0 <= c && c < m;
        };

        auto satisfied = [&](int r, int c) {
            int neigh = 0;
            for (int i = 0; i < 4; ++i) {
                int nr = r + dr[i];
                int nc = c + dc[i];
                if (isInGrid(nr, nc) && greedy[nr][nc] == '^') {
                    neigh++;
                }
            }
            return neigh >= 2;
        ...
        while (!q.empty()) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (isInGrid(nr, nc) && greedy[nr][nc] == '^' && !satisfied(nr, nc)) {
                    greedy[nr][nc] = '.';
                    q.emplace(nr, nc);
                }
            }
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                if (map[i][j] == '^' && greedy[i][j] != '^') {
                    possible = false;
                }
            }
        }

        io.println(possible ? "Possible" : "Impossible");
        if (possible) {
            for (auto line : greedy) {
                io.println(line);
            }
        }
    });
