...
int main() {
    int n = io.scanInt();
    auto graph = io.scanTree(n);
    
    std::function<void(int, int)> eraseParent = [&](int start, int parent) {
        auto found = std::find(graph[start].begin(), graph[start].end(), parent);
        if (found != graph[start].end()) {
            std::swap(*found, *std::prev(graph[start].end()));
            graph[start].pop_back();
        }

        for (int next : graph[start]) {
            eraseParent(next, start);
        }
    };
    eraseParent(1, 0);

    auto dp = createTensor<int>(-1, 2, n+1);
    std::function<int(int, int)> getMinEarly = [&](int parentIsEarly, int curr) -> int {
        int& result = dp[parentIsEarly][curr];
        if (result != -1) {
            return result;
        }
        ...
        if (parentIsEarly) {
            int cand2 = 0;
            for (int next : graph[curr]) {
                cand2 += getMinEarly(0, next);
            }
            result = std::min(result, cand2);
        }

        return result;
    };

    io.println(getMinEarly(1, 1));
