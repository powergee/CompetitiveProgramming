...
int main() {
    io.google([&]() {
        ...
        auto parties = io.nextInts(n);
        std::sort(parties.begin(), parties.end(), std::greater<int>());

        int answerDouble = 0;
        for (int i = 0; i < m-1; ++i) {
            answerDouble += parties[i] * 2;
        }
        std::vector<int> selected;
        for (int i = m-1; i < n; ++i) {
            selected.push_back(parties[i]);
        }
        std::sort(selected.begin(), selected.end());
        int median;
        if (selected.size() % 2) {
            median = selected[selected.size()/2] * 2;
        } else {
            median = selected[selected.size()/2-1] + selected[selected.size()/2];
        }
        answerDouble += median;
        
        if (answerDouble % 2) {
            std::cout << answerDouble / 2 << ".5\n";
        } else {
            std::cout << answerDouble / 2 << ".0\n";
        }
    });
