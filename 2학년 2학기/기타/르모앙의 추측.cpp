...
int main()
{
    ...
    std::vector<Complex> oddPrimes(500001, Complex(0, 0)), twices(1000001, Complex(0, 0));
    for (int i = 0; i < 1000001; ++i)
    {
        if (isPrime[i])
        {
            twices[i] = Complex(1, 0);
            if (i % 2)
                oddPrimes[(i+1)/2] = Complex(1, 0);
        }
    }
    std::vector<Complex> conv = getConvolution(oddPrimes, twices);
    ...
    while (T--)
    {
        int n;
        scanf("%d", &n);
        printf("%lld\n", (long long)std::round(conv[(n-1)/2+1].real()));
    }
