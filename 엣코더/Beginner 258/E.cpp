...
int main() {
    auto [n, q, x] = io.nexts<Long, 3>();
    auto w = io.nextLongs(n);
    Long wSum = std::accumulate(w.begin(), w.end(), 0LL);
    auto wAug = w;
    wAug.resize(w.size()*2);
    for (int i = w.size(); i < wAug.size(); ++i) {
        wAug[i] = w[i%w.size()];
    }
    for (int i = 1; i < wAug.size(); ++i) {
        wAug[i] += wAug[i-1];
    }
    ...
    while (true) {
        if (visited[curr]) {
            loopStart = visited[curr];
            break;
        } else {
            visited[curr] = start.size();
            start.push_back(curr);
            count.push_back(0);

            count.back() += (x/wSum)*n;
            Long spareWeight = x - ((x/wSum)*wSum);
            if (spareWeight > 0) {
                int end = std::lower_bound(wAug.begin(), wAug.end(), wAug[curr]-w[curr]+spareWeight) - wAug.begin();
                count.back() += end - curr + 1;
                curr = (end+1) % n;
            }
        }
    }
    Long loopLength = count.size() - loopStart;
    
    while (q--) {
        Long k = io.nextLong();
        if (k <= count.size()) {
            io.println(count[k-1]);
        } else {
            Long inLoop = (k-loopStart-1);
            io.println(count[loopStart + (inLoop % loopLength)]);
        }
    }
