...
int main() {
    auto [n, k] = io.nexts<Long, 2>();
    auto count = io.nextLongs(n);

    std::vector<std::pair<Long, Long>> buckets;
    for (int i = 0; i < n; ++i) {
        if (count[i]) {
            buckets.emplace_back(count[i], i);
        }
    }

    std::sort(buckets.begin(), buckets.end(), std::greater<std::pair<Long, Long>>());
    Long reduced = 0;
    while (k) {
        Long smallest = buckets.back().first-reduced;
        if (smallest*buckets.size() < k) {
            reduced += smallest;
            k -= smallest*buckets.size();
            count[buckets.back().second] = 0;
            buckets.pop_back();
        } else {
            Long end = k % buckets.size();
            std::sort(buckets.begin(), buckets.end(), [](std::pair<Long, Long> p, std::pair<Long, Long> q) {
                return p.second < q.second;
            });
            for (int i = 0; i < buckets.size(); ++i) {
                count[buckets[i].second] -= reduced;
                count[buckets[i].second] -= (k/buckets.size()) + (i < end ? 1 : 0);
            }
            k = 0;
        }
    }
