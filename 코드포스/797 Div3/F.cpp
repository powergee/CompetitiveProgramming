...
int main() {
    ...
    io.codeforces([&]() {
        int n = io.scanInt();
        auto origin = " " + io.scanToken();
        auto trans = io.scanInts(n, 1);

        std::vector<bool> visited(n+1, false);
        auto measureCycle = [&](int start) -> int {
            std::vector<int> comp;
            ...
            do {
                visited[curr] = true;
                comp.push_back(curr);
                values.push_back(origin[curr]);
                curr = trans[curr];
            } while (curr != start);

            std::deque<char> mut = values;
            int count = 0;
            do {
                mut.push_front(mut.back());
                mut.pop_back();
                ++count;
            } while (mut != values);
            return count;
        ...
        for (int v = 1; v <= n; ++v) {
            if (!visited[v]) {
                cycles.push_back(measureCycle(v));
            }
        }

        Long answer = cycles[0];
        for (int i = 1; i < cycles.size(); ++i) {
            answer = std::lcm(answer, cycles[i]);
        }
        io.println(answer);
    });
