...
int main() {
    ...
    Memo getMaxSum(-1LL, n+1, m+1);
    getMaxSum = [&](int start, int selected) -> Long {
        if (start == n) {
            return selected == m ? 0 : INT64_MIN/2;
        }
        if (selected == m) {
            return 0;
        }
        Long result = INT64_MIN/2;
        result = std::max(result, getMaxSum(start+1, selected));
        result = std::max(result, arr[start]*(selected+1) + getMaxSum(start+1, selected+1));
        return result;
    };

    io.println(getMaxSum(0, 0));
