...
int main() {
    io.codeforces([&]() {
        ...
        auto init = io.nextInts(m);
        std::sort(init.begin(), init.end());

        std::deque<Range> alive;
        for (int i = 0; i < m; ++i) {
            int curr = init[i]-1;
            int next = init[(i+1)%m]-1;
            if (((curr+1)%n) == next) {
                continue;
            }
            int l = (curr+1)%n;
            int r = (next-1+n)%n;
            int dist = (l <= r ? r-l : (n-l)+r);
            alive.emplace_back(dist, l, r);
        }
        std::sort(alive.begin(), alive.end(), std::greater<Range>());

        int rescued = 0, time = 0;
        while (alive.size()) {
            auto [dist, l, r] = alive.front();
            alive.pop_front();
            if (dist+1 <= 2*time) {
                continue;
            }
            int currL = (l+time) % n;
            int currR = (r-time+n) % n;
            int currCnt = (currL <= currR ? currR-currL+1 : n-currL+currR+1);
            // block right
            rescued++;
            time++;
            if (currCnt <= 2) {
