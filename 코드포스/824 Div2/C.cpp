...
int main() {
    io.codeforces([&]() {
        int n = io.nextInt();
        auto encrypted = io.nextToken();
        std::string decryped;
        decryped.reserve(n);
        int involved = 0;
        DisjointSet dsu(26);
        std::vector<int> prev(26, -1), next(26, -1);
        for (int i = 0; i < n; ++i) {
            int c1 = encrypted[i] - 'a';

            if (next[c1] == -1) {
                ++involved;
                for (int c2 = 0; c2 < 26; ++c2) {
                    if (prev[c2] == -1 && (involved == 26 || dsu.findParent(c1) != dsu.findParent(c2))) {
                        next[c1] = c2;
                        prev[c2] = c1;
                        dsu.unionNodes(c1, c2);
                        break;
                    }
                }
            }
            assert(next[c1] != -1);
            decryped.push_back(char(next[c1] + 'a'));
        }
        io.println(decryped);
    });
