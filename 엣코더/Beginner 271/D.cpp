...
int main() {
    ...
    const int IMPOSSIBLE = INT32_MAX;
    Memo findProperSide(-1, n+1, s+1);
    findProperSide = [&](int start, int sum) {
        if (start == n) {
            return sum == s ? 0 : IMPOSSIBLE;
        }
        if (sum+cards[start].first <= s) {
            int next = findProperSide(start+1, sum+cards[start].first);
            if (next != IMPOSSIBLE) {
                return (int)'H';
            }
        }
        if (sum+cards[start].second <= s) {
            int next = findProperSide(start+1, sum+cards[start].second);
            if (next != IMPOSSIBLE) {
                return (int)'T';
            }
        }
    ...
    if (IMPOSSIBLE == findProperSide(0, 0)) {
        io.println("No");
    } else {
        io.println("Yes");
        int sum = 0;
        std::string answer;
        for (int i = 0; i < n; ++i) {
            int side = findProperSide(i, sum);
            answer += char(side);
            sum += side == int('H') ? cards[i].first : cards[i].second;
        }
        io.println(answer);
    }
