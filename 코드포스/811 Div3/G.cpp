...
int main() {
    io.codeforces([&]() {
        int n = io.nextInt();
        Tensor<2, std::tuple<int, Long, Long>> graph(n+1);
        for (int i = 2; i <= n; ++i) {
            auto [p, a, b] = io.nextLongs<3>();
            graph[p].emplace_back(i, a, b);
        }

        std::vector<int> answer(n+1);
        std::vector<Long> bSum = {0};
        bSum.reserve(n+2);
        std::function<void(int, int, Long)> initAnswer = [&](int start, int parent, Long aSum) {
            answer[start] = std::upper_bound(bSum.begin(), bSum.end(), aSum) - bSum.begin();
            for (auto [next, a, b] : graph[start]) {
                if (next != parent) {
                    bSum.push_back(bSum.back()+b);
                    initAnswer(next, start, aSum+a);
                    bSum.pop_back();
                }
            }
        };
