...
int main() {
    io.codeforces([&]() {
        auto [n, k, b, s] = io.nexts<Long, 4>();
        std::vector<Long> answer(n, (b/n)*k);
        Long spareBeauty = b - (b/n)*n;
        s -= (b/n)*k*n;
        if (s < 0 || s < spareBeauty*k) {
            io.println(-1);
        } else {
            for (int i = 0; i < spareBeauty; ++i) {
                answer[i] += k;
                s -= k;
            }
            if (s/n+((s%n) ? 1 : 0) >= k) {
