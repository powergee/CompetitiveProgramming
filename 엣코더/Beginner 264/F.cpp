...
class DisjointSet {
private:
    std::vector<int> parent, size, value;
public:
    ...
    void setValue(int x, int newSize) {
        value[x] = newSize;
    }
    ...
    int getCompValue(int x) {
        return value[findParent(x)];
    }
};

int main() {
    auto [h, w] = io.nexts<int, 2>();
    auto rowCost = io.nextLongs(h);
    auto colCost = io.nextLongs(w);
    auto map = io.nextTokens(h);

    auto inverse = [](char color) {
        return color == '0' ? '1' : '0';
    };

    Memo getCost(-1LL, 2, 2, h, w);
    getCost = [&](int rowInv, int colInv, int row, int col) -> Long {
        if (row == h-1 && col == w-1) {
            return 0;
        }
        Long result = INT64_MAX/2;
        char currColor = ((rowInv+colInv)%2) ? inverse(map[row][col]) : map[row][col];
        // go right
        if (col+1 < w) {
            char rightColor = rowInv ? inverse(map[row][col+1]) : map[row][col+1];
            if (rightColor == currColor) {
                result = std::min(result, getCost(rowInv, 0, row, col+1));
            } else {
                result = std::min(result, colCost[col+1] + getCost(rowInv, 1, row, col+1));
            }
        }

        // go down
        if (row+1 < h) {
            char downColor = colInv ? inverse(map[row+1][col]) : map[row+1][col];
            if (downColor == currColor) {
                result = std::min(result, getCost(0, colInv, row+1, col));
            } else {
                result = std::min(result, rowCost[row+1] + getCost(1, colInv, row+1, col));
            }
        }
    ...
    for (int ri = 0; ri < 2; ++ri) {
        for (int ci = 0; ci < 2; ++ci) {
            answer = std::min(answer, (ri ? rowCost[0] : 0) + (ci ? colCost[0] : 0) + getCost(ri, ci, 0, 0));
        }
    }
    io.println(answer);
