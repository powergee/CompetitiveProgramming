...
int main() {
    auto [s, t] = io.nexts<std::string, 2>();
    
    auto getProfile = [](const std::string& str) {
        std::vector<std::pair<char, int>> result;
        for (char ch : str) {
            if (result.empty() || result.back().first != ch) {
    ...
    auto p1 = getProfile(s);
    auto p2 = getProfile(t);
    ...
    io.printYes([&]() -> bool {
        ...
        for (int i = 0; i < p1.size(); ++i) {
            ...
            if (p1[i].second == 1 && p2[i].second > 1) {
                return false;
            }
        }
        return true;
