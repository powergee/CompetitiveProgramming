...
int main() {
    auto [n, x] = io.nexts<int, 2>();
    auto arr = io.nextInts(n);
    auto count = countElements(arr);
    std::deque<std::pair<int, int>> d(count.begin(), count.end());

    while (d.size()) {
        auto p1 = d.front();
        d.pop_front();

        if (d.size() > 0 && d.front().first == p1.first) {
            auto p2 = d.front();
            d.pop_front();
            d.push_front({p1.first, p1.second+p2.second});
        } else if (p1.second % (p1.first+1) == 0) {
            d.push_front({p1.first+1, p1.second/(p1.first+1)});
        } else {
            d.push_front(p1);
            break;
        }
