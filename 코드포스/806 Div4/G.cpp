...
int main() {
    io.codeforces([&]() {
        ...
        auto coins = io.nextLongs(n);

        Tensor<2, Long> dp = createTensor(Long(INT64_MIN/2), n, 31);
        for (int decay = 0; decay < 31; ++decay) {
            dp[n-1][decay] = std::max(
                (coins[n-1] >> decay) - k,
                (coins[n-1] >> (decay+1))
            );
        }
        for (int start = n-2; start >= 0; --start) {
            // decay < 30
            for (int decay = 0; decay < 30; ++decay) {
                dp[start][decay] = std::max(
                    dp[start+1][decay] - k + (coins[start] >> decay),
                    dp[start+1][decay+1] + (coins[start] >> (decay+1))
                );
            }
            // decay == 30
            dp[start][30] = std::max(
                dp[start+1][30] - k + (coins[start] >> 30),
                dp[start+1][30] + (coins[start] >> 31)
            );
        }
        io.println(dp[0][0]);
    });
