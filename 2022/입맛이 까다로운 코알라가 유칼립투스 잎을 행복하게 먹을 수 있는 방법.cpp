...
int main() {
    ...
    auto [a, b, c] = io.nexts<int, 3>();

    std::vector<std::pair<int, int>> leaf;
    for (int i = 0; i < n; ++i) {
        auto [x, y] = io.nexts<int, 2>();
        leaf.emplace_back(x, y);
    }

    // leaf index, remaining day, curr poison
    Memo getMaxHappiness(-1LL, n+1, m+1, a+1);
    getMaxHappiness = [&](int leafIndex, int remDay, int currPoison) -> Long {
        if (leafIndex == n || remDay == 0) {
            return 0;
        }

        Long result = 0;
        // 현재 잎을 먹고 잔다.
        if (currPoison+leaf[leafIndex].first <= a) {
            Long sub = leaf[leafIndex].second + getMaxHappiness(leafIndex+1, remDay-1, std::max(currPoison+leaf[leafIndex].first-b, 0));
            result = std::max(result, sub);
        }
        // 현재 잎은 건너 뛴다.
        {
            Long sub = getMaxHappiness(leafIndex+1, remDay, currPoison);
            result = std::max(result, sub);
        }
        // 그냥 잔다.
        {
            Long sub = getMaxHappiness(leafIndex, remDay-1, std::max(currPoison-b, 0));
            result = std::max(result, sub);
        }
        return result;
    };

    io.println(getMaxHappiness(0, m, c));
