...
int main() {
    ...
    io.codeforces([&]() {
        ...
        std::set<int> cands(arr.begin(), arr.end());
        ...
        for (int i = 0; i < n-2; ++i) {
            if (*cands.begin() != arr[i]) {
                answer[i] = *cands.begin();
                cands.erase(cands.begin());
            } else {
                auto it = std::next(cands.begin());
                answer[i] = *it;
                cands.erase(it);
            }
        }

        std::vector<int> spare(cands.begin(), cands.end());
        if (spare[0] != arr[n-2] && spare[1] != arr[n-1]) {
            answer[n-2] = spare[0];
            answer[n-1] = spare[1];
        } else {
            answer[n-2] = spare[1];
