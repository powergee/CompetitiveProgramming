...
int main() {
    io.google([&]() {
        int n = io.nextInt();
        auto s = io.nextInts(n);
        int m = io.nextInt();
        auto k = io.nextInts(m);
        
        auto findNearestLeft = [&](std::vector<int>& arr) -> Tensor<2, int> {
            int max = *std::max_element(arr.begin(), arr.end());
            Tensor<2, int> result = createTensor(0, max+1, arr.size());
            for (int v = 1; v <= max; ++v) {
                result[v][0] = (arr[0] == v ? 0 : -1);
                for (int i = 1; i < arr.size(); ++i) {
                    result[v][i] = (arr[i] == v ? i : result[v][i-1]);
                }
            }
            return result;
        };

        auto findNearestRight = [&](std::vector<int>& arr) -> Tensor<2, int> {
            int arrSize = arr.size();
            int max = *std::max_element(arr.begin(), arr.end());
            Tensor<2, int> result = createTensor(0, max+1, arrSize);
            for (int v = 1; v <= max; ++v) {
                result[v][arrSize-1] = (arr[arrSize-1] == v ? arrSize-1 : arrSize);
        ...
        auto left = findNearestLeft(k);
        auto right = findNearestRight(k);

        Tensor<2, int> dp = createTensor(-1, n, m);
        std::function<int(int, int)> getMinTime = [&](int stringIdx, int fingerIdx) -> int {
            if (stringIdx >= n) {
                return 0;
            }
            int& result = dp[stringIdx][fingerIdx];
            if (result != -1) {
                return result;
            }
            result = INT32_MAX;
            int leftNear = left[s[stringIdx]][fingerIdx];
            if (leftNear != -1) {
                result = std::min(result, fingerIdx-leftNear + getMinTime(stringIdx+1, leftNear));
            }
            int rightNear = right[s[stringIdx]][fingerIdx];
            if (rightNear != m) {
                result = std::min(result, rightNear-fingerIdx + getMinTime(stringIdx+1, rightNear));
            }
            return result;
        };
        ...
        for (int i = 0; i < m; ++i) {
            answer = std::min(answer, getMinTime(0, i));
        }
        io.println(answer);
    });
