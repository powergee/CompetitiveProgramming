...
int main() {
    ...
    std::vector<int> maxBegin(m+1, -1); // 아직 지나지 못한 것의 최대 지점
    std::vector<int> maxEnd(m+1, -1); // 이미 지난 것의 최대 지점
    ...
    for (int i = 0; i < n; ++i) {
        ...
        ranges[i] = { a, b };
        maxBegin[a] = a;
        maxEnd[a+1] = std::max(maxEnd[a+1], b);
        minEnd = std::min(minEnd, b);
    }

    for (int p = m-1; p >= 1; --p) {
        maxBegin[p] = std::max(maxBegin[p], maxBegin[p+1]);
    }
    for (int p = 1; p <= m; ++p) {
        maxEnd[p] = std::max(maxEnd[p], maxEnd[p-1]);
    }
    
    std::vector<Long> answer(m+2, 0);
    for (int p = 1; p <= minEnd; ++p) {
        int endStart = std::max(maxBegin[p], maxEnd[p]);
        if (p <= endStart && endStart <= m) {
            int minLength = endStart - p + 1;
            int maxLength = m - p + 1;
            answer[minLength]++;
            answer[maxLength+1]--;
        }
