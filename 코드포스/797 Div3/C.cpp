...
struct Job {
    int given, finished, index;

    std::tuple<int, int, int> toTuple() const {
        return std::tuple<int, int, int>(given, finished, index);
    }

    bool operator<(const Job& j) const {
        return toTuple() < j.toTuple();
    }
};
...
int main() {
    ...
    io.codeforces([&]() {
        int n = io.scanInt();
        auto s = io.scanInts(n);
        auto f = io.scanInts(n);
        ...
        for (int i = 0; i+1 < n; ++i) {
            jobs[i+1].given = std::max(jobs[i].finished, jobs[i+1].given);
        }
        for (int i = 0; i < n; ++i) {
            d[jobs[i].index] = jobs[i].finished - jobs[i].given;
        }
        io.println(d);
    });
