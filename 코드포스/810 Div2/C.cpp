...
int main() {
    io.codeforces([&]() {
        auto [n, m, k] = io.nexts<Long, 3>();
        auto pigments = io.nextLongs(k);
        std::sort(pigments.begin(), pigments.end());

        auto isPossible = [&](Long perLine, Long lines) -> bool {
            auto currPigm = pigments;
            Long currLineIdx = 0;
            bool has3Lines = false;

            while (currLineIdx < lines) {
                Long margin = lines-currLineIdx;
                while (currPigm.size() && currPigm.back() / perLine < 2) {
                    currPigm.pop_back();
                }
                if (currPigm.empty()) {
                    return false;
                ...
                if (currPigm.back() / perLine <= margin) {
                    if (currPigm.back() / perLine >= 3) {
                        has3Lines = true;
                    }
                    currLineIdx += currPigm.back() / perLine;
                    currPigm.back() = 0;
                } else if (margin >= 2 || has3Lines) {
                    currLineIdx = lines;
                } else {
                    return false;
                }
        ...
        io.printYes(isPossible(n, m) || isPossible(m, n));
