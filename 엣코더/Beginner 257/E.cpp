...
int main() {
    int n = io.scanInt();
    auto prices = io.scanInts(9);

    std::vector<int> maxLength(n+1), selection(n, -1);
    for (int budget = 0; budget <= n; ++budget) {
        maxLength[budget] = 0;
        for (int i = 0; i < 9; ++i) {
            if (prices[i] <= budget) {
                int newLength = 1 + maxLength[budget-prices[i]];
                if (maxLength[budget] <= newLength) {
                    maxLength[budget] = newLength;
                    selection[budget] = i;
                }
            }
        }
    ...
    answer.reserve(n);
    int curr = n;
    while (selection[curr] != -1) {
        answer += char('1' + selection[curr]);
        curr -= prices[selection[curr]];
    }
    io.println(answer);
