...
int main() {
    io.codeforces([&]() {
        ...
        auto isPossible = [&](int minDiameter) -> bool {
            int count = 0;
            auto curr = arr;
            for (int i = 0; i < n; ++i) {
                if (2*curr[i] < minDiameter) {
                    ++count;
                    curr[i] = 1e9;
                }
            }
            if (count > k) {
                ...
                int m = *std::min_element(curr.begin(), curr.end());
                int diameter = 0;
                for (int i = 0; i < n-1; ++i) {
                    diameter = std::max(diameter, std::min(2*m, std::min(curr[i], curr[i+1])));
                }
                return minDiameter <= diameter;
            } else if (k-count == 1) {
                return minDiameter <= *std::max_element(curr.begin(), curr.end());
            } else {
                return true;
            }
        };
