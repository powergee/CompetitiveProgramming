...
int main() {
    ...
    std::vector<int> reduce(3*n, 0);

    auto markDiff = [&](int l, int r, int add) {
        reduce[r+1] -= add;
        reduce[l] += add;
    };

    for (int i = 0; i < n; ++i) {
        markDiff(i+1, i+arr[i]/m, m);
        if (arr[i] % m) {
            markDiff(i+arr[i]/m+1, i+arr[i]/m+1, arr[i]%m);
        }
    }

    for (int i = 1; i < reduce.size(); ++i) {
        reduce[i] += reduce[i-1];
    }
    
    std::vector<int> total = arr;
    for (int i = 1; i < n; ++i) {
        total[i] += total[i-1];
        total[i] -= reduce[i];
    }
    ...
    while (q--) {
        auto [c, x] = io.nexts<int, 2>();
        if (c == 1) {
            io.println(total[x-1]);
        } else {
            io.println(reduce[x-1]);
        }
    }
