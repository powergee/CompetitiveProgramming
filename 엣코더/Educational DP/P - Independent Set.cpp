...
int main() {
    int n = io.nextInt();
    auto graph = io.nextGraph(n, n-1);

    std::function<void(int, int)> buildRootedTree = [&](int start, int parent) {
        auto found = std::find(graph[start].begin(), graph[start].end(), parent);
        if (found != graph[start].end()) {
            graph[start].erase(found);
        }
        for (int next : graph[start]) {
            buildRootedTree(next, start);
        }
    };
    buildRootedTree(1, 0);

    Memo countCases(-1LL, n+1, 2);
    countCases = [&](int start, int color) -> Long {
        if (graph[start].empty()) {
            return 1;
        }
        Long result = 1;
        for (int next : graph[start]) {
            Long sub = countCases(next, 0);
            if (color == 0) {
                sub += countCases(next, 1);
            }
            result *= sub;
            result %= MOD;
        }
    ...
    std::cout << (countCases(1, 0) + countCases(1, 1)) % MOD;
