...
int main() {
    io.codeforces([&]() {
        ...
        Tensor<2, Long> rectByW(1001);
        Tensor<2, Long> rectByWSum(1001);
        for (int i = 0; i < n; ++i) {
            auto [h, w] = io.nexts<int, 2>();
            rectByW[w].push_back(h);
        }
        for (int i = 0; i < rectByW.size(); ++i) {
            std::sort(rectByW[i].begin(), rectByW[i].end());
            rectByWSum[i] = rectByW[i];
            for (int j = 1; j < rectByW[i].size(); ++j) {
                rectByWSum[i][j] += rectByWSum[i][j-1];
            }
        }

        while (q--) {
            auto [hs, ws, hb, wb] = io.nexts<int, 4>();
            Long answer = 0;
            for (int w = ws+1; w < wb; ++w) {
                if (rectByW[w].empty()) {
                    continue;
                }
                auto l = std::upper_bound(rectByW[w].begin(), rectByW[w].end(), hs);
                if (l == rectByW[w].end()) {
                    continue;
                }
                int lIdx = l - rectByW[w].begin();

                auto r = std::upper_bound(rectByW[w].rbegin(), rectByW[w].rend(), hb, std::greater<int>());
                if (r == rectByW[w].rend()) {
                    continue;
                }
                int rIdx = (int(rectByW[w].size())-1) - (r - rectByW[w].rbegin());
                assert(lIdx < rectByWSum[w].size());
                assert(rIdx < rectByWSum[w].size());
                if (lIdx <= rIdx) {
                    answer += (lIdx == 0 ? rectByWSum[w][rIdx] : (rectByWSum[w][rIdx] - rectByWSum[w][lIdx-1])) * w;
                }
            }
            io.println(answer);
        }
