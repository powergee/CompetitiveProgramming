...
int main() {
    auto [n, w] = io.nexts<int, 2>();
    auto weights = io.nextInts(n);

    std::set<int> answer;
    auto updateAnswer = [&](int s) {
        if (s <= w) {
            answer.insert(s);
        }
    };
    ...
    for (int i = 0; i < n; ++i) {
        updateAnswer(weights[i]);
        for (int j = i+1; j < n; ++j) {
            updateAnswer(weights[i] + weights[j]);
            for (int k = j+1; k < n; ++k) {
                updateAnswer(weights[i] + weights[j] + weights[k]);
            }
        }
    }
