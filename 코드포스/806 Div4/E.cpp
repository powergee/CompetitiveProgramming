...
struct Matrix {
    ...
    Matrix(std::vector<std::string> input) {
        h = w = input.size();
        raw.resize(input.size());
        for (int i = 0; i < input.size(); ++i) {
            raw[i].resize(input[i].size());
...
int main() {
    io.codeforces([&]() {
        int n = io.nextInt();
        auto origin = Matrix(io.nextTokens(n));

        int answer = 0;
        for (int i = 0; i < n/2; ++i) {
            int sum = 0;
            sum += origin.raw[i][i] ? 1 : 0;
            sum += origin.raw[i][n-1-i] ? 1 : 0;
            sum += origin.raw[n-1-i][i] ? 1 : 0;
            sum += origin.raw[n-1-i][n-1-i] ? 1 : 0;
            answer += std::min(sum, 4-sum);
        }

        Matrix cands[4];
        cands[0] = origin;
        for (int i = 1; i < 4; ++i) {
            cands[i] = cands[i-1].rotateRight();
        }
        ...
        for (int i = 0; i+1 <= n-(i+2); ++i) {
            ...
            for (int j = l; j <= r; ++j) {
                int count = 0;
                for (int k = 0; k < 4; ++k) {
                    count += cands[k].raw[i][j] ? 1 : 0;
                }
                answer += std::min(count, 4-count);
            }
        }
