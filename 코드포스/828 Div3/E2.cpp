...
int main() {
    io.codeforces([&]() {
        ...
        // Find x (a < x <= c) such that:
        // b*x < lcm(a*b, x) * ((b*x)/lcm(a*b, x) + 1) <= d*x
        // then, the answer is { x, lcm(a*b, x) * ((b*x)/lcm(a*b, x) + 1) / x }

        // lcm(a*b, x) = x*q(x)
        // q(x) = lcm(a*b, x)/x = (a*b)/gcd(a*b, x)
        // b < q(x) * (floor(b/q(x)) + 1) <= d

        auto aDiv = getDividers(a);
        auto bDiv = getDividers(b);
        std::set<Long> abDiv;
        for (auto v1 : aDiv) {
            for (auto v2 : bDiv) {
                abDiv.insert(v1);
        ...
        io.println([&]() -> std::pair<Long, Long> {
            for (auto div : abDiv) {
                Long x = (a/div+1) * div;
                if (a < x && x <= c) {
                    Long qx = (a*b) / std::gcd(a*b, x);
                    Long y = qx * (b/qx + 1);
                    if (b < y && y <= d) {
                        assert(a < x && x <= c && b < y && y <= d && (x*y) % (a*b) == 0);
                        return { x, y };
                    }
                }
            }
