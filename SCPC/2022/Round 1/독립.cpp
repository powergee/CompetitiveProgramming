...
int main() {
    io.samsung([&]() {
        ...
        Tensor<2, int> count = createTensor(0, n, n);

        std::function<std::pair<int, int>(int, int, bool)> findJoint = [&](int r, int c, bool wentDown) -> std::pair<int, int> {
            if (wentDown) {
                if (map[r][c-1] == '.') {
                    return {r, c};
                } else if (r < n-1 && map[r+1][c] == '.') {
                    return findJoint(r+1, c, true);
                } else {
                    return findJoint(r, c+1, false);
                }
            } else {
                if (r < n-1 && map[r+1][c] == '.') {
                    return findJoint(r+1, c, true);
        ...
        for (int r = n-1; r >= 0; --r) {
            for (int c = n-1; c >= 0; --c) {
                ...
                count[r][c] = 1;
                if (r < n-1 && map[r+1][c] == '.' && c < n-1 && map[r][c+1] == '.') {
                    count[r][c] += count[r+1][c] + count[r][c+1];
                    auto joint = findJoint(r, c+1, false);
                    count[r][c] -= count[joint.first][joint.second];
                } else if (r < n-1 && map[r+1][c] == '.') {
                    count[r][c] += count[r+1][c];
                } else if (c < n-1 && map[r][c+1] == '.') {
