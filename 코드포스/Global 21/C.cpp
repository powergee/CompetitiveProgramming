...
int main() {
    io.codeforces([&]() {
        auto [n, m] = io.scans<Long, 2>();
        auto src = io.scanLongs(n);
        int k = io.scanLong();
        auto dst = io.scanLongs(k);

        auto isPossible = [&]() -> bool {
            std::deque<std::pair<Long, Long>> srcDeque;
            std::deque<Long> dstDeque(dst.begin(), dst.end());
            for (Long s : src) {
                std::pair<Long, Long> count = { s, 1 };
                while (count.first % m == 0) {
                    count = { count.first / m, count.second * m };
                }
                if (!srcDeque.empty() && srcDeque.back().first == count.first) {
                    srcDeque.back().second += count.second;
                } else {
                    srcDeque.push_back(count);
                }
            }
            
            while (!dstDeque.empty()) {
                Long need = dstDeque.front();
                dstDeque.pop_front();

                if (srcDeque.empty()) {
                    return false;
                }
                std::pair<Long, Long> supply = srcDeque.front();
                srcDeque.pop_front();

                if (need >= supply.first &&
                    need % supply.first == 0 &&
                    isPower((need / supply.first), m) &&
                    (need / supply.first) <= supply.second) {

                    supply.second -= (need / supply.first);
                    if (supply.second) {
                        srcDeque.push_front(supply);
                    }
                } else {
                    return false;
                }
            }
            return srcDeque.empty();
        };
