...
int main() {
    io.codeforces([&]() {
        ...
        int curr = 0, answer = 0;
        while (curr < n-1) {
            std::function<bool(int)> isPossible;
            std::function<int(int)> findValidTeleport;
            if (arr[curr] < arr[curr+1]) {
                // arr[curr] must be min
                isPossible = [&](int target) -> bool {
                    int min = minTree.query(curr, target);
                    return arr[curr] == min;
                };
                findValidTeleport = [&](int target) -> int {
                    return indexMap[maxTree.query(curr, target)];
                };
            } else {
                // arr[curr] must be max
                isPossible = [&](int target) -> bool {
                    int max = maxTree.query(curr, target);
                    return arr[curr] == max;
                };
                findValidTeleport = [&](int target) -> int {
                    return indexMap[minTree.query(curr, target)];
                };
            }
