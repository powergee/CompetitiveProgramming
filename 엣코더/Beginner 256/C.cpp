...
int main() {
    auto rowSum = io.nextInts(3);
    auto colSum = io.nextInts(3);

    Tensor<2, int> board = createTensor(-1, 3, 3);
    std::function<int(int, int)> countCases = [&](int r, int c) {
        if (r == 3) {
            return std::count(rowSum.begin(), rowSum.end(), 0) == 3 && std::count(colSum.begin(), colSum.end(), 0) == 3 ? 1 : 0;
        }
        int index = r*3 + c;
        int result = 0;
        for (int v = 1; v <= std::min(rowSum[r], colSum[c]); ++v) {
            board[r][c] = v;
            rowSum[r] -= v;
            colSum[c] -= v;
            int nr = (index+1)/3;
            int nc = (index+1)%3;
            if (nr == r || rowSum[r] == 0) {
                result += countCases((index+1)/3, (index+1)%3);
            }
            rowSum[r] += v;
            colSum[c] += v;
        }
