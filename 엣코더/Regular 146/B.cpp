...
int main() {
    ...
    auto cands = arr;
    for (Long e = 32; e >= 0; --e) {
        std::vector<std::pair<Long, Long>> costs(cands.size());
        for (int i = 0; i < cands.size(); ++i) {
            if (cands[i] & (1LL<<e)) {
                costs[i] = { 0LL, cands[i] };
            } else {
                costs[i] = { (1LL<<e)-(cands[i]&((1LL<<e)-1LL)), cands[i] };
            }
        }

        std::sort(costs.begin(), costs.end(), [](std::pair<Long, Long> p1, std::pair<Long, Long> p2) {
        ...
        if (need <= m) {
            ...
            cands.resize(0);
            for (int i = 0; i < k; ++i) {
                if (costs[i].second & (1LL<<e)) {
                    cands.push_back(costs[i].second);
                } else {
                    cands.push_back(0LL);
                }
            }
