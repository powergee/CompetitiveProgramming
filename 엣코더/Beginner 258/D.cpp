...
int main() {
    auto [n, x] = io.nexts<Long, 2>();

    std::vector<std::pair<Long, Long>> time(n);
    for (int i = 0; i < n; ++i) {
        auto [a, b] = io.nexts<Long, 2>();
        time[i] = { a, b };
    }

    Long prevSum = 0, answer = INT64_MAX;
    for (int i = 0; i < n; ++i) {
        Long repeat = x-i;
        if (repeat <= 0) {
            break;
        }
        answer = std::min(answer, prevSum + time[i].first + (time[i].second)*repeat);
        prevSum += time[i].first + time[i].second;
    }
