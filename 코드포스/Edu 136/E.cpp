...
int main() {
    int n = io.nextInt();
    auto map = io.nextTokens(2);
    map[0].resize(n+1, '0');
    map[1].resize(n+1, '0');

    Memo getMaxDirt(-1, 2, n+1, 2);
    // diagRemoved: 현재 대각선 방향에 있는 칸이 제거된 상태라면 1
    getMaxDirt = [&](int row, int col, int diagRemoved) -> int {
        if (col == n) {
            return 0;
        }
        int status = map[row][col]-'0';
        int answer = status + getMaxDirt(row, col+1, 0);
        if (map[!row][col] == '1' && !diagRemoved) {
            answer = std::max(answer, status+1 + getMaxDirt(!row, col+1, 1));
        }
        return answer;
    };

    io.println(getMaxDirt(0, 0, 0));
