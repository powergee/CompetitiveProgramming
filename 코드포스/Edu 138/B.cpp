...
int main() {
    io.codeforces([&]() {
        int n = io.nextInt();
        auto health = io.nextLongs(n);
        auto spell = io.nextLongs(n);
        io.println(std::accumulate(health.begin(), health.end(), 0LL) + std::accumulate(spell.begin(), spell.end(), 0LL) - *std::max_element(spell.begin(), spell.end()));
    });
