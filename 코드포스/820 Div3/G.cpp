...
int main() {
    io.codeforces([&]() {
        ...
        Memo countCases(-1LL, s.size()+1, minCost+1);
        countCases = [&](int start, int currCost) -> Long {
            if (start == s.size()) {
                return currCost == minCost ? 1 : 0;
            } else if (matched[start]) {
                if (currCost == minCost) {
                    return INT64_MIN;
                }
                Long result = 0;
                for (size_t i = start; i < std::min(start+t.size(), s.size()); ++i) {
                    if (matched[i]) {
                        auto sub = countCases(i+t.size(), currCost+1);
                        if (sub != INT64_MIN) {
                            result += sub;
                            result %= 1000000007LL;
                        }
                ...
                return countCases(start+1, currCost);
            }
        };

        io.println(minCost, countCases(0, 0));
    });
