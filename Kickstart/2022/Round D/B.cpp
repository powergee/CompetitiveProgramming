...
int main() {
    io.google([&]() {
        int n = io.nextInt();
        auto a = io.nextLongs(n, 1);
        int m = io.nextInt();
        auto b = io.nextLongs(m, 1);
        int k = io.nextInt();
        ...
        auto findMaxSum = [&](const std::vector<Long>& sum, int count) -> Long {
            int arrSize = sum.size()-1;
            if (count <= 0) {
                return 0;
            }
            if (count >= arrSize) {
            ...
            Long answer = 0;
            int exclude = arrSize - count;
            for (int i = 1; i+exclude-1 <= arrSize; ++i) {
                int j = i+exclude-1;
                answer = std::max(answer, sum.back() - (sum[j] - sum[i-1]));
            }
            return answer;
        };
        ...
        for (int aCnt = 0; aCnt <= n; ++aCnt) {
            int bCnt = k-aCnt;
            if (bCnt >= 0) {
                answer = std::max(answer, (findMaxSum(aSum, aCnt) + findMaxSum(bSum, bCnt)));
            }
        }
