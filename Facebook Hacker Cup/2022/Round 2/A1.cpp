...
int main() {
    io.google([&]() {
        ...
        int n = t.size();
        t = " " + t;
        std::vector<std::valarray<int>> count(n+1, std::valarray<int>(26));
        ...
        while (q--) {
            ...
            auto isValid = [&](std::valarray<int> left, std::valarray<int> right) {
                auto diff = std::abs(right-left);
                int one = 0, zero = 0;
                for (int i = 0; i < 26; ++i) {
                    if (diff[i] == 0) {
                        zero++;
                ...
                return (one == 1 && zero == 25);
            };

            bool valid = isValid(count[m-1] - count[l-1], count[r] - count[m-1]) || isValid(count[m] - count[l-1], count[r] - count[m]);
            if (valid) {
                ++answer;
            }
        }
