...
int main() {
    io.codeforces([&]() {
        ...
        std::function<int(int)> countMinQueries = [&](int start) -> int {
            if (dp[start] != -1) {
                return dp[start];
            }
            ...
            for (int next : tree[start]) {
                if (dp[next] == -1) {
                    dp[start] += countMinQueries(next);
                    if (dp[next] == 0) {
                        ++leaf;
                    }
                }
            }
            dp[start] += std::max(0, leaf-1);
            return dp[start];
        };
        io.println(countMinQueries(entry));
    });
