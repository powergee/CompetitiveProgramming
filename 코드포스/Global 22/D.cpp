...
int main() {
    io.codeforces([&]() {
        int n = io.nextInt();
        auto b = io.nextInts(n, 1);
        
        std::map<int, std::vector<int>> bInv;
        for (int i = 1; i <= n; ++i) {
            bInv[b[i]].push_back(i);
        }

        std::vector<int> block;
        bool currLower = false;
        int lowerCount = 0;
        if (bInv.contains(0)) {
            currLower = false;
            block.reserve(bInv[0].size());
            for (int i : bInv[0]) {
                block.emplace_back(i);
            }
        } else if (bInv.contains(n+1)) {
            currLower = true;
            block.reserve(bInv[n+1].size());
            for (int i : bInv[n+1]) {
                block.emplace_back(i);
            }
        } else {
